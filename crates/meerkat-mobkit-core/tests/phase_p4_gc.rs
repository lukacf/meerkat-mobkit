...
#[test]
fn gc_superseded_rows_sends_correct_dml_query() {
...
#[test]
fn truncate_sessions_sends_correct_truncate_query() {
    let server = MockHttpServer::start(vec![MockHttpResponse::json(json!({
        "jobComplete": true
    }))]);
    let store = build_mock_adapter(&server);
...
#[test]
fn gc_superseded_rows_returns_affected_row_count() {
    let server = MockHttpServer::start(vec![MockHttpResponse::json(json!({
        "jobComplete": true,
...
#[test]
fn gc_superseded_rows_returns_zero_when_no_affected_rows_field() {
    let server = MockHttpServer::start(vec![MockHttpResponse::json(json!({
        "jobComplete": true
    }))]);
    let store = build_mock_adapter(&server);
...
#[test]
fn gc_superseded_rows_propagates_api_error_on_non_success_status() {
...
#[test]
fn truncate_sessions_propagates_api_error_on_non_success_status() {
    let server = MockHttpServer::start(vec![MockHttpResponse {
        status_code: 500,
        content_type: "application/json".to_string(),
...
#[test]
fn gc_superseded_rows_requires_project_id() {
...
#[test]
fn truncate_sessions_requires_access_token() {
    let store = BigQuerySessionStoreAdapter::new_native("gc_dataset", "gc_table")
        .with_project_id("gc-project");
