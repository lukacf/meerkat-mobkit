...
```json
{
  "checks": [
    ...
    "Verified the existing gh keyring account lukacf has push permission for lukacf/meerkat without exposing credential values or changing global auth. Push uses per-command env -u GH_TOKEN -u GITHUB_TOKEN git -c credential.helper= -c 'credential.helper=!gh auth git-credential' push -u origin HEAD, with normal hooks enabled."
  ],
  "caveats": [
    ...
    "Push remains in progress under normal hooks; PR metadata will be filled after it finishes."
  ]
}
```
