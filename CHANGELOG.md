...
- **Incremental continuity persistence: identity-member turns append
  ...
  **Upgrade note — the ledger bump is one-way, and deliberately NOT applied
  ...
  maintenance fence; `--dry-run` announces the pending bump. Once stamped, binaries older than this release refuse the
  file at open with `SchemaFromTheFuture` — recovery is restoring
  `continuity.*` from backup, so **back up `continuity.*` before the first
  post-upgrade turn.** The doctor reports frozen archives under the new
