...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct MobpackSourceResult {
    pub filename: String,
    pub media_type: String,
    pub mob_toml: String,
    pub source_files: Vec<MobpackSourceFile>,
...
pub fn source_mobpack(params: &Value) -> Result<MobpackSourceResult, String> {
    let rendered = render_mobpack_source(params)?;
    Ok(MobpackSourceResult {
        filename: rendered.filename,
        media_type: MOBPACK_MEDIA_TYPE.to_string(),
        mob_toml: rendered.mob_toml,
        source_files: source_files_from_archive_files(&rendered.files),
        validation: rendered.validation,
        source: "mobkit/mobpacks/source".to_string(),
    })
}
...
pub fn export_mobpack(params: &Value) -> Result<MobpackExportResult, String> {
    ...
    let bytes = encode_deployable_mobpack_archive(&rendered.files)?;
...
"#
    ...
    #[test]
    fn previews_editor_document_source_without_exporting_archive_payload() {
        let mut document = document_with_real_launch_modes();
        document.mob_toml = None;
        let result =
            source_mobpack(&json!({ "document": document })).expect("preview rendered source");

        assert!(result.validation.ok, "{:?}", result.validation.diagnostics);
        assert_eq!(result.source, "mobkit/mobpacks/source");
        ...
        assert!(
            result
                ...
                .any(|file| file.path == "mobkit/mob.toml"
                    && file.text.as_deref() == Some(result.mob_toml.as_str()))
        );
        let serialized = serde_json::to_value(&result).expect("serialize source result");
        assert!(
            serialized.get("content_base64").is_none(),
            "source preview must not return the full archive payload"
        );
    }
...
"#
    ...
    fn schema_response_exposes_flow_control_contract_for_editor_hydration() {
        ...
        assert_eq!(
            ...
            json!("rendering mob.toml from mobkit/mobpacks/source...")
        );
        assert_eq!(
