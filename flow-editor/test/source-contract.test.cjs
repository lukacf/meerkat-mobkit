...
assert.match(controller, /function authoringOperationFromIntent[\s\S]*agent\.updateMember[\s\S]*type:\s*"update_member"[\s\S]*graph\.moveNode[\s\S]*type:\s*"move_graph_node"/, "controller plane must translate Agent and Graph UI intents into MobKit authoring operations");
assert.match(controller, /function authoringReplacementFromIntent[\s\S]*basic\.updateStep[\s\S]*operationType:\s*"update_flow_step"[\s\S]*settings\.updateDeploy[\s\S]*operationType:\s*"update_deploy_settings"/, "controller plane must translate Basic and settings intents into MobKit replacement operations");
