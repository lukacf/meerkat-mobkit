...
assert.match(controller, /function graphStructureSignature\(instances, edges\) \{[\s\S]*graphSignatureFor\(instances,\s*edges,\s*\{\s*includeLayout:\s*true\s*\}\)/, "Graph-to-Basic sync signature must include projection grid placement because graphToFlow derives step order and primitives from col/row");
