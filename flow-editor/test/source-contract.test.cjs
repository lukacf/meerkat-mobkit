...
assert.match(builder, /commitFlow\(result\.flow,\s*\{\},\s*"insert_flow_step"\);[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\);[\s\S]*commitFlow\(result\.flow,\s*\{\},\s*"delete_flow_step"\);/, "Basic flow step insert/delete must apply controller-projected flow, selection, and picker transitions through named MobKit operations");
