...
assert.match(app, /exportCurrentSourceDocument = async \(requestToken\) => \{[\s\S]*MobKitFlowController\.exportDocument\(document\)[\s\S]*if \(!sourceProjectionIsCurrent\(requestToken\)\) return null;[\s\S]*persistCurrentOutcome\(projection\)/, "stale mobkit/mobpacks/export responses must not write source, validation, or flow-row state after an edit");
...
assert.match(app, /const handleValidate = async \(\) => \{[\s\S]*const requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.validateDocument\(document\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale validation responses must not write validation, stage, or flow-row state after an edit");
...
assert.match(app, /const handlePublish = async \(\) => \{[\s\S]*const requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.exportDocument\(document\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale publish/export responses must not write published validation state after an edit");
assert.match(app, /const handleDeploy = async \(\{ execute \}\) => \{[\s\S]*const requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.deployDocument\(document, \{ execute \}\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale deploy responses must not write validation, stage, or flow-row state after an edit");
