...
assert.match(mobpackRust, /fn apply_projected_authoring_document_operation\([\s\S]*document: &mut MobpackDocument[\s\S]*operation\.get\("document"\)[\s\S]*serde_json::from_value\(next_document\.clone\(\)\)/, "MobKit named projected operations must apply operation.document on the server side");
