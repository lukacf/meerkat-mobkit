...
assert.match(builder, /commitFlow\(result\.flow\);[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\);/, "Basic flow step insert/delete must apply controller-projected flow, selection, and picker transitions through MobKit replacement");
...
assert.match(agents, /const renameField = \(fieldId, oldName, newName\) => \{[\s\S]*schemaFieldRenameCascadePatch\([\s\S]*applySchemaCascade\(result\);/, "Agent Editor schema-field rename commits must apply controller-cleaned schemas, Basic flow, and Graph edges together through MobKit replacement");
...
assert.match(agents, /function SchemaEditor[\s\S]*const deleteSchema = \(\) => \{[\s\S]*applySchemaCascade\(result, result\.selection\);[\s\S]*onClick=\{deleteSchema\}/, "Agent Editor schema deletes must apply controller-cleaned Basic flow and Graph edges as one MobKit authoring mutation");
...
assert.match(agents, /const deleteField = \(fieldId\) => \{[\s\S]*schemaFieldDeleteCascadePatch\([\s\S]*applySchemaCascade\(result\);/, "Agent Editor schema-field deletes must apply controller-cleaned Basic flow and Graph edges through MobKit replacement");
...
assert.match(tweaksBlock, /applyAuthoringReplacement\(\{\s*deploySettings:\s*next\s*\}\)/, "Tweaks deploy settings mutations must round-trip through MobKit replacement");
assert.match(tweaksBlock, /applyAuthoringReplacement\(\{\s*mobSettings:\s*next\s*\}\)/, "Tweaks mob settings mutations must round-trip through MobKit replacement");
