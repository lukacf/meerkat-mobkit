...
assert.match(app, /let document;[\s\S]*document = currentMobKitDocument\(\);[\s\S]*catch \(error\) \{[\s\S]*return \{ ok: false, error: error\?\.message \|\| String\(error\) \};[\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*currentDraftGuard\(\)\)/, "Agent add-control operations must send only the current MobKit-returned deployable document into MobKit validation with draft revision guards");
