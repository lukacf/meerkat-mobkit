...
assert.match(app, /const hasRuntimeRows = Array\.isArray\(catalogPayload\?\.runtime_flows\) && catalogPayload\.runtime_flows\.length > 0;[\s\S]*if \(\(!Array\.isArray\(registryPayload\?\.rows\) \|\| registryPayload\.rows\.length === 0\) && !hasRuntimeRows\) \{[\s\S]*MobKitFlowController\.createDocument\(\{[\s\S]*template:\s*"blank"[\s\S]*trigger:\s*"MobKit editor startup draft"[\s\S]*\},\s*rpcOptions\)/, "app shell must create a real MobKit blank draft only when neither saved rows nor runtime flow projections exist");
...
assert.match(app, /const applyMobKitAuthoringOperation = React\.useCallback\(\(operation\) =>\s*authoringOperationRunner\.current\(operation\), \[\]\)/, "app shell must invoke the controller-owned MobKit authoring runner instead of applying operations inline");
...
assert.match(controller, /graphToFlow:\s*"mobkit\/mobpacks\/graph_to_flow"[\s\S]*async function graphToFlowDocument\(document,\s*options = \{\}\)[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("graphToFlow"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "controller plane must expose MobKit graph-to-flow projection through mobkit/mobpacks/graph_to_flow with AbortSignal out-of-band");
