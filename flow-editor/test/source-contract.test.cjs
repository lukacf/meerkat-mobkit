...
assert.match(mobpackRust, /pub fn mobpack_catalogs_response[\s\S]*"authoring_provider":\s*standalone_authoring_provider\("mobkit\/mobpacks\/catalogs"\)[\s\S]*"runtime_binding":\s*"unbound"/, "Composed MobKit catalogs must mark standalone authoring as an unbound runtime provider");
...
assert.match(app, /const applyMobKitAuthoringOperation = async \(operation\) => \{[\s\S]*MobKitFlowController\.applyAuthoringOperationDocument\(document, operation,\s*currentDraftGuard\(\)\)[\s\S]*authoringProjectionFromOperationResult\(result/, "app shell must round-trip authoring mutations through MobKit before applying projected state");
assert.match(app, /applyAuthoringOperationDocument\(document,\s*\{[\s\S]*\.\.\.operation,[\s\S]*\},\s*currentDraftGuard\(\)\)/, "projected authoring replacements must carry the same MobKit draft revision guard");
...
assert.match(controller, /async function graphProjectionDocument\(document,\s*options = \{\}\)[\s\S]*rpcMethod\("graphProjection"\)/, "controller plane must expose MobKit graph projection through mobkit/mobpacks/graph_projection");
assert.match(controller, /graphToFlow:\s*"mobkit\/mobpacks\/graph_to_flow"[\s\S]*async function graphToFlowDocument\(document,\s*options = \{\}\)[\s\S]*rpcMethod\("graphToFlow"\)/, "controller plane must expose MobKit graph-to-flow projection through mobkit/mobpacks/graph_to_flow");
...
assert.match(mobpackRust, /fn guarded_document_actions_reject_stale_draft_revision[\s\S]*mobkit\/mobpacks\/validate draft revision conflict[\s\S]*mobkit\/mobpacks\/source draft revision conflict[\s\S]*mobkit\/mobpacks\/export draft revision conflict[\s\S]*mobkit\/mobpacks\/deploy_command draft revision conflict[\s\S]*mobkit\/mobpacks\/deploy draft revision conflict/, "MobKit tests must prove guarded validate/source/export/deploy-command/deploy requests reject stale draft revisions");
...
assert.match(app, /const document = currentMobKitDocument\(\);[\s\S]*applyAuthoringOperationDocument\(document, operation,\s*currentDraftGuard\(\)\)/, "Agent add-control operations must send the full current MobKit-returned deployable document into MobKit validation with draft revision guards");
