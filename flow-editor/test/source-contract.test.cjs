...
assert.match(app, /applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*\{[\s\S]*\.\.\.currentDraftGuard\(\),[\s\S]*catalogSnapshot:\s*catalogs\.catalogSnapshot/, "app shell must bind authoring operations to the loaded MobKit catalog snapshot");
...
assert.match(app, /MobKitFlowController\.authoringOperationAvailability\(catalogs\.authoringOperations,\s*translatedOperation\?\.type\)/, "app shell must gate direct authoring operations after controller intent translation");
...
assert.match(app, /const applyMobKitAuthoringOperation = async \(operation\) => \{[\s\S]*authoringOperationFromIntent\(operation\)[\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*\{[\s\S]*\.\.\.currentDraftGuard\(\),[\s\S]*catalogSnapshot:\s*catalogs\.catalogSnapshot[\s\S]*\}\)[\s\S]*authoringProjectionFromOperationResult\(result/, "app shell must round-trip controller-translated authoring mutations through MobKit with draft and catalog snapshot guards before applying projected state");
...
assert.match(app, /let document;[\s\S]*document = currentMobKitDocument\(\);[\s\S]*catch \(error\) \{[\s\S]*return \{ ok: false, error: error\?\.message \|\| String\(error\) \};[\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*\{[\s\S]*\.\.\.currentDraftGuard\(\),[\s\S]*catalogSnapshot:\s*catalogs\.catalogSnapshot/, "Agent add-control operations must send only the current MobKit-returned deployable document into MobKit validation with draft revision and catalog snapshot guards");
