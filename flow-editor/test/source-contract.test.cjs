...
assert.match(app, /operationType:\s*"insert_graph_node"[\s\S]*operation:\s*\{[\s\S]*instances:\s*inserted\.instances[\s\S]*edges:\s*inserted\.edges[\s\S]*flow:\s*inserted\.flow/, "Graph quick insert must send explicit MobKit graph payloads, not only a replacement document");
...
assert.match(mobpackRust, /"type":\s*"insert_graph_node"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["instance_or_instances"\]/, "MobKit operation catalog must advertise graph node insert as MobKit-owned");
