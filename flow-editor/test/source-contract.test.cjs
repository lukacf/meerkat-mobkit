...
assert.match(app, /const abort = new AbortController\(\);[\s\S]*MobKitFlowController\.graphProjectionDocument\(\{[\s\S]*instances:\s*\[\],[\s\S]*edges:\s*\[\],[\s\S]*frames:\s*\[\],[\s\S]*\},\s*\{ signal:\s*abort\.signal \}\)[\s\S]*graphProjectionFromMobKitResult\(projectionResult\)[\s\S]*if \(abort\.signal\.aborted\) return;[\s\S]*abort\.abort\(\)/, "Basic-to-Graph live projection must use the MobKit graph projection API and abort stale projection RPCs");
...
assert.match(app, /graphProjectionDocument\([\s\S]*\},\s*\{ signal:\s*abort\.signal \}\)[\s\S]*\},\s*\[flow,\s*editorMode,\s*contract,\s*studio\.members\]\);/, "Basic-to-Graph live projection must rerun when Agent editor member definitions change");
