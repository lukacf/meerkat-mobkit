...
assert.match(app, /deployCommandPreviewForDocument\(document,\s*\{ signal:\s*abort\.signal \}\)/, "deploy command preview refreshes must abort stale MobKit preview RPCs");
...
assert.match(app, /React\.useEffect\(\(\) => \{[\s\S]*let cancelled = false;[\s\S]*setDeployCommandPreview\(""\);[\s\S]*buildMobKitProjectedDocument\(\)[\s\S]*MobKitFlowController\.deployCommandPreviewForDocument\(document,\s*\{ signal:\s*abort\.signal \}\)/, "deploy command preview must clear stale MobKit RPC output before requesting a document-backed preview");
