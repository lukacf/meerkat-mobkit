...
assert.match(graph, /studioAddMemberPatch\(\{\s*members,\s*contract:\s*authoring\.contract\s*\}/, "Graph state hook must pass the MobKit schema contract into member-add validation");
assert.match(graph, /studioUpdateMemberPatch\(\{\s*members,\s*contract:\s*authoring\.contract\s*\}/, "Graph state hook must pass the MobKit schema contract into member-update validation");
...
assert.match(app, /operationType:\s*"insert_graph_node"[\s\S]*operation:\s*\{\s*instance:\s*next\.instance\s*\}/, "Graph instance adds must send the inserted instance payload to MobKit");
...
assert.match(graph, /const deleteInstance = \(id\) => \{[\s\S]*studioDeleteInstancePatch[\s\S]*setEdges\(next\.edges\);[\s\S]*return next;/, "Graph state hook must return controller-projected instance-delete transitions");
