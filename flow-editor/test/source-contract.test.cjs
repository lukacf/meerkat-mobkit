...
assert.match(graphEditorBlock, /MobKitFlowController\.graphCellXY/, "Graph canvas must ask the controller plane for grid cell coordinates");
...
assert(!/function\s+(?:dynGrid|cellXYFor|nodeBox|portOut|portIn|edgePath|midpoint)\b|const\s+NODE_[WH]\b/.test(graph), "Graph canvas must not define local geometry helpers or node dimensions");
