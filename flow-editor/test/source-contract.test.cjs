...
assert.match(agents, /const renameField = \(fieldId, oldName, newName\) => \{[\s\S]*schemaFieldRenameCascadePatch\([\s\S]*applySchemaCascade\(result,[\s\S]*"rename_schema_field"\);/, "Agent Editor schema-field rename commits must apply controller-cleaned schemas, Basic flow, and Graph edges together through named MobKit operation");
...
assert.match(agents, /function SchemaEditor[\s\S]*const deleteSchema = \(\) => \{[\s\S]*applySchemaCascade\(result, result\.selection,\s*"delete_schema"\);[\s\S]*onClick=\{deleteSchema\}/, "Agent Editor schema deletes must apply controller-cleaned Basic flow and Graph edges as one named MobKit authoring mutation");
...
assert.match(agents, /const deleteField = \(fieldId\) => \{[\s\S]*schemaFieldDeleteCascadePatch\([\s\S]*applySchemaCascade\(result,[\s\S]*"delete_schema_field"\);/, "Agent Editor schema-field deletes must apply controller-cleaned Basic flow and Graph edges through named MobKit operation");
...
assert.match(tweaksBlock, /applyAuthoringReplacement\(\{\s*operationType:\s*"update_deploy_settings",\s*deploySettings:\s*next\s*\}\)/, "Tweaks deploy settings mutations must round-trip through named MobKit operation");
assert.match(tweaksBlock, /operationType:\s*field === "roleWiring" \? "update_role_wiring" : "update_mob_settings"[\s\S]*mobSettings:\s*next/, "Tweaks mob settings mutations must round-trip through named MobKit operations");
