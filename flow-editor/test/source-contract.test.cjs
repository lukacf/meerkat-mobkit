...
assert.match(controller, /function authoringOperationFromIntent[\s\S]*agent\.updateMember[\s\S]*type:\s*"update_member"/, "controller plane must translate Agent UI intents into MobKit authoring operations");
assert.match(controller, /function authoringReplacementFromIntent[\s\S]*basic\.updateStep[\s\S]*operationType:\s*"update_flow_step"[\s\S]*graph\.moveNode[\s\S]*operationType:\s*"move_graph_node"[\s\S]*settings\.updateDeploy[\s\S]*operationType:\s*"update_deploy_settings"/, "controller plane must translate Basic, Graph, and settings intents into MobKit replacement operations");
assert.doesNotMatch(builder + "\n" + graph + "\n" + agents, /operationType|applyAuthoringReplacement|applyAuthoringOperation|add_agent_definition|update_member|add_member_tool|remove_member_tool|assign_member_schema|delete_member|add_schema|update_schema|rename_schema|delete_schema|toggle_member_skill|remove_member_skill|create_inline_skill|update_flow_step|apply_flow_step_edit|insert_flow_step|delete_flow_step|add_input_param|update_input_param|delete_input_param|move_graph_node|connect_graph_nodes|insert_graph_node|delete_graph_node/, "Basic, Graph, and Agent UI files must emit intent callbacks rather than MobKit operation payloads");
...
assert.match(app, /addInstance:\s*\(instance\) => \{[\s\S]*return applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"graph\.insertNode"/, "Graph add instance callback must return the MobKit authoring operation result through an intent");
assert.match(app, /deleteEdge:\s*\(id\) => \{[\s\S]*return applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"graph\.deleteEdge"/, "Graph delete edge callback must return the MobKit authoring operation result through an intent");
...
assert.match(agentEditorBlock, /applyAgentReplacementIntent\(\{[\s\S]*intent:\s*"agent\.deleteMember"[\s\S]*memberId:\s*member\.id[\s\S]*selection/, "Agent delete must submit only member_id through a controller-translated MobKit intent");
...
assert.match(app, /intent:\s*"graph\.insertNode"[\s\S]*pick,[\s\S]*cell:\s*addAt/, "Graph quick insert must send semantic pick and cell intent payloads to MobKit");
...
assert.match(app, /intent:\s*"graph\.insertNode"[\s\S]*instance/, "Graph instance adds must send the inserted instance intent payload to MobKit");
...
assert.match(app, /intent:\s*"graph\.updateNode"[\s\S]*instanceId:\s*id,[\s\S]*patch/, "Graph instance updates must send instanceId and patch to MobKit");
assert.match(graph, /intent:\s*"graph\.moveNode"[\s\S]*instanceId:\s*drag\.instId[\s\S]*cell[\s\S]*originalCell:/, "Graph drag moves must send instanceId, cell, and originalCell to MobKit");
...
assert.match(app, /intent:\s*"graph\.deleteNode"[\s\S]*instanceId:\s*id/, "Graph instance deletes must send instanceId to MobKit");
...
assert.match(app, /intent:\s*"graph\.connectNodes"[\s\S]*operation,\s*selection:/, "Graph edge adds must send a MobKit graph connect intent");
...
assert.match(graph, /intent:\s*"graph\.connectNodes"[\s\S]*fromId:\s*conn\.fromId,[\s\S]*toId:\s*closest\.dataset\.instId/, "Graph port connections must send semantic endpoint payloads to MobKit");
...
assert.match(app, /intent:\s*"graph\.updateEdge"[\s\S]*edgeId:\s*id,[\s\S]*patch/, "Graph edge updates must send edgeId and patch to MobKit");
assert.match(app, /intent:\s*"graph\.deleteEdge"[\s\S]*edgeId:\s*id/, "Graph edge deletes must send edgeId to MobKit");
...
assert.match(app, /intent:\s*"graph\.deleteNode"[\s\S]*instanceId:\s*selection\.id[\s\S]*selection:\s*nextSelection/, "keyboard graph node deletes must submit only instanceId and UI selection");
assert.match(app, /intent:\s*"graph\.deleteEdge"[\s\S]*edgeId:\s*selection\.id[\s\S]*selection:\s*nextSelection/, "keyboard graph edge deletes must submit only edgeId and UI selection");
...
assert.match(app, /if \(!result\.changed\) return;[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"system\.reconcileContractRefs"/, "app shell must round-trip contract reconciliation changes through a MobKit-owned intent");
...
assert.match(app, /const buildMobKitProjectedDocument = async \(overrides = \{\}\) => \{[\s\S]*if \(editorMode !== "advanced"\) \{[\s\S]*const document = currentMobKitDocument\(overrides\);[\s\S]*return \{ document, requestToken \};[\s\S]*intent:\s*"system\.syncGraphToFlow"[\s\S]*return \{ document: result\?\.document \|\| authoringDocumentRef\.current \|\| buildDocument\(overrides\), requestToken \};/, "Basic and Graph API actions must start from the current MobKit-owned document, with Graph mode refreshing through MobKit sync_graph_to_flow");
...
assert.match(app, /reconcileConditionFieldAvailability\(\{[\s\S]*const flowChanged = result\.flow !== flow;[\s\S]*const edgesChanged = result\.edges !== studio\.edges;[\s\S]*if \(!flowChanged && !edgesChanged\) return;[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"system\.reconcileConditionFields"/, "schema-backed condition availability rewrites must round-trip Basic/Graph cleanup through a MobKit-owned intent");
...
assert.match(app, /result\.mobSettings !== mobSettings[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"system\.reconcileMembers"/, "app shell must round-trip mob-level profile setting reconciliation through MobKit-owned member reconciliation");
...
assert.match(agentEditorBlock, /const changeSchema = \(rawSchema\) => \{[\s\S]*applyAgentReplacementIntent\(\{[\s\S]*intent:\s*"agent\.assignSchema"[\s\S]*memberId:\s*member\.id,[\s\S]*schemaId:\s*rawSchema[\s\S]*onChange=\{e => changeSchema\(e\.target\.value\)\}/, "Agent schema writes must submit only member_id and schema_id through a typed MobKit intent");
...
assert.match(app, /intent:\s*"graph\.insertNode"[\s\S]*pick,[\s\S]*cell:\s*addAt/, "Graph quick insertion must pass only semantic pick and cell into MobKit");
