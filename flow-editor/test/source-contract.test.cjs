...
assert.match(mobpackRust, /let success = execute\s*&& status_code == Some\(0\)\s*&& run_envelope\s*\.as_ref\(\)\s*\.is_none_or\(\|envelope\| envelope\.status == "completed"\);/, "mobkit/mobpacks/deploy must report success only after rkat mob run executed, exited 0, and any run envelope reports completed");
