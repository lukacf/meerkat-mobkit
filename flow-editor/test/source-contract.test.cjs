...
const mobpackRust = fs.readFileSync(path.join(root, "..", "crates", "meerkat-mobkit", "src", "mobpack.rs"), "utf8");
const flowEditorHttpRust = fs.readFileSync(path.join(root, "..", "crates", "meerkat-mobkit", "src", "http_flow_editor.rs"), "utf8");
const rpcRust = fs.readFileSync(path.join(root, "..", "crates", "meerkat-mobkit", "src", "rpc.rs"), "utf8");
const unifiedRuntimeHttpRust = fs.readFileSync(path.join(root, "..", "crates", "meerkat-mobkit", "src", "unified_runtime", "http.rs"), "utf8");
const flowEditorBinRust = fs.readFileSync(path.join(root, "..", "crates", "meerkat-mobkit", "src", "bin", "mobkit_flow_editor.rs"), "utf8");
...
const applyAuthoringProjectionBlock = (app.match(/const applyAuthoringDocumentProjection = \(projection\) => \{[\s\S]*?const currentMobKitDocument/) || [""])[0];
const graphEdgeCanvasBlock = (graph.match(/const edgeEls = state\.edges\.map[\s\S]*?const canvasAdornments =/) || [""])[0];
...
assert.match(browserInteractionsSmoke, /\.port-out[\s\S]*document\.querySelectorAll\("\.edge"\)[\s\S]*reachable\.has\(to\)/, "browser interaction smoke must cover rendered graph port-to-edge creation");
...
assert.doesNotMatch(app, /authoringOperationQueue|enqueueMobKitAuthoringTask/, "app shell must not own MobKit authoring mutation queueing");
assert.match(controller, /function createAuthoringOperationRunner[\s\S]*let queue = Promise\.resolve\(\)[\s\S]*authoringOperationFromIntent\(operation\)[\s\S]*authoringOperationAvailability\([\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,[\s\S]*authoringProjectionFromOperationResult\(result/, "controller plane must serialize and apply MobKit authoring mutations before projecting accepted documents");
assert.match(controller, /const enqueuedRevision = hooks\.getCurrentRevision\?\.\(\);[\s\S]*runOperation\(operation,\s*enqueuedRevision\)/, "controller-owned authoring runner must bind queued operations to the document revision active at enqueue time");
assert.match(controller, /runOperation = async \(operation,\s*enqueuedRevision\) => \{[\s\S]*!hooks\.isRevisionCurrent\(enqueuedRevision\)[\s\S]*MobKit authoring operation result is stale/, "queued authoring operations must fail stale before reading a newer hydrated document");
...
assert.match(controller, /async function applyAuthoringOperationDocument\(document,\s*operation,\s*options = \{\}\)[\s\S]*expectedSnapshotId[\s\S]*expected_catalog_snapshot_id:\s*expectedSnapshotId/, "controller RPC wrapper must thread the loaded catalog snapshot id into authoring operations");
assert.match(controller, /function createAuthoringOperationRunner[\s\S]*getDraftGuard\?\.\(\)[\s\S]*catalogSnapshot:\s*hooks\.getCatalogSnapshot\?\.\(\)/, "controller-owned authoring runner must bind operations to the current draft and loaded MobKit catalog snapshot");
assert.match(app, /createAuthoringOperationRunner\(\{[\s\S]*getDraftGuard:\s*\(\)\s*=>\s*authoringRunnerContext\.current\.currentDraftGuard\(\)[\s\S]*getCatalogSnapshot:\s*\(\)\s*=>\s*authoringRunnerContext\.current\.catalogs\?\.catalogSnapshot/, "app shell must expose draft and catalog guard callbacks to the controller-owned authoring runner");
assert.match(mobpackRust, /fn catalog_deployability[\s\S]*"command":\s*"rkat mob run"/, "MobKit catalogs must expose deployability metadata tied to rkat mob run");
assert.match(mobpackRust, /fn with_catalog_snapshot[\s\S]*"deploy_command":\s*"rkat mob run"/, "MobKit catalogs must expose deterministic snapshot metadata for deploy-context provenance");
assert.match(mobpackRust, /fn enforce_expected_catalog_snapshot[\s\S]*operation_uses_authoring_catalog[\s\S]*catalog snapshot conflict/, "MobKit apply_operation must reject stale catalog snapshots for catalog-backed authoring mutations");
assert.match(mobpackRust, /"add_agent_definition" => \{[\s\S]*apply_add_agent_definition_operation\(&mut document,\s*operation,\s*runtime\)/, "MobKit add-agent mutations must resolve definitions through the runtime-backed authoring catalog when available");
assert.match(mobpackRust, /fn mobpack_agent_definition_by_id\([\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*mobpack_catalogs_response_with_runtime\(runtime\)/, "MobKit agent-definition lookup must use the same runtime catalog snapshot exposed to the UI");
...
assert.match(mobpackRust, /fn runtime_authoring_provider[\s\S]*"id":\s*"unified_runtime"[\s\S]*"runtime_binding":\s*"bound"[\s\S]*"deploy_target"[\s\S]*"command":\s*"rkat mob run"/, "MobKit catalog APIs must expose a runtime-bound authoring provider for live UnifiedRuntime surfaces");
assert.match(mobpackRust, /pub fn mobpack_schema_response\(\) -> Value \{[\s\S]*mobpack_schema_response_with_runtime\(None\)[\s\S]*pub fn mobpack_schema_response_with_runtime\([\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*deploy_settings_schema_for_provider/, "MobKit schema API must have standalone and runtime-bound deploy-settings paths");
assert.match(mobpackRust, /fn deploy_settings_schema_for_provider\([\s\S]*deploy_target[\s\S]*"UnifiedRuntime\.authoring_provider\.deploy_target"/, "MobKit deploy settings schema must derive runtime-backed options from the runtime authoring provider");
assert.match(rpcRust, /"mobkit\/mobpacks\/schema"[\s\S]*Some\(mobpack_runtime_catalog_state\(runtime\)\.await\)[\s\S]*"mobkit\/mobpacks\/schema" => Ok\(crate::mobpack::mobpack_schema_response_with_runtime\([\s\S]*runtime,[\s\S]*\)\)/, "UnifiedRuntime RPC routing must attach runtime state to mobpack schema responses");
...
assert.match(mobpackRust, /pub struct MobpackRuntimeCatalogState \{[\s\S]*runtime_flow_rows: Vec<Value>[\s\S]*runtime_agent_definition_sources: Vec<Value>[\s\S]*runtime_skill_realms: Vec<Value>/, "MobKit runtime catalog state must carry projected runtime flow rows, profile-definition sources, and skill realms for the Flow Editor");
assert.match(mobpackRust, /pub fn runtime_flow_registry_rows_from_definition\([\s\S]*MobDefinition[\s\S]*runtime_flow_registry_row_from_definition[\s\S]*project_definition_to_editor_document_for_flow/, "MobKit must project loaded runtime MobDefinition flows into real Flow Editor registry rows");
assert.match(mobpackRust, /pub fn runtime_agent_definition_sources_from_definition\([\s\S]*MobDefinition[\s\S]*runtime_agent_definition_source_from_definition[\s\S]*"mobkit\/runtime\/agent-definitions"/, "MobKit must project loaded runtime MobDefinition profiles into real Agent Editor definition sources");
assert.match(mobpackRust, /pub fn runtime_skill_realms_from_definition\([\s\S]*skill_realms_from_definition\(definition\)[\s\S]*mobkit\/runtime\/skills/, "MobKit must project loaded runtime MobDefinition skills into the runtime-backed skill catalog");
assert.match(mobpackRust, /fn skill_realms_response_with_runtime_state\([\s\S]*runtime_skill_realms\.clone\(\)[\s\S]*authoring_skill_realms_response/, "MobKit skill catalog must prefer runtime skill realms before generic authoring skills");
assert.match(mobpackRust, /mobpack_agent_definitions_response_with_runtime\([\s\S]*runtime_agent_definition_sources[\s\S]*authoring_agent_definition_mobpack_sources[\s\S]*agent_definition_catalog/, "Agent definition catalogs must prefer runtime MobDefinition profile sources before generic authoring fallbacks");
assert.match(mobpackRust, /"runtime_flows": runtime[\s\S]*runtime_flow_rows\.clone\(\)/, "Composed MobKit catalogs must expose runtime flow projections when runtime-backed");
assert.match(mobpackRust, /pub fn list_mobpack_drafts_with_runtime\([\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*runtime_flow_rows\.clone\(\)[\s\S]*chain\(draft_rows\)/, "MobKit list API must merge runtime flow projections ahead of local draft rows");
assert.match(mobpackRust, /pub fn get_mobpack_draft_with_runtime\([\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*runtime_flow_rows\.iter\(\)[\s\S]*"runtime_backed": true/, "MobKit get API must open runtime flow projections directly by id");
assert.match(rpcRust, /"mobkit\/mobpacks\/list"[\s\S]*"mobkit\/mobpacks\/get"[\s\S]*Some\(mobpack_runtime_catalog_state\(runtime\)\.await\)/, "UnifiedRuntime RPC routing must attach runtime flow state to mobpack list/get");
assert.match(rpcRust, /"mobkit\/mobpacks\/list" => crate::mobpack::list_mobpack_drafts_with_runtime\(params, runtime\)[\s\S]*"mobkit\/mobpacks\/get" => crate::mobpack::get_mobpack_draft_with_runtime\(params, runtime\)/, "MobKit authoring RPC helper must route list/get through runtime-aware APIs");
assert.match(flowEditorHttpRust, /protected_flow_editor_router_with_runtime_catalog[\s\S]*runtime_catalog: MobpackRuntimeCatalogState[\s\S]*flow_editor_rpc_router_with_runtime_catalog[\s\S]*runtime_catalog: Option<MobpackRuntimeCatalogState>/, "Protected Flow Editor router must accept MobKit runtime catalog state instead of serving only standalone catalogs");
...
assert.match(app, /loadSchema\(rpcOptions\)[\s\S]*configureAuthoringMethodsFromSchema\(schema\)[\s\S]*loadCatalogs\(rpcOptions\)/, "app shell must load schema first, then use schema.commands to request dynamic catalogs");
...
assert.match(controller, /function authoringOperationFromIntent[\s\S]*system\.reconcileMembers[\s\S]*type:\s*"reconcile_members"[\s\S]*agent\.updateMember[\s\S]*type:\s*"update_member"[\s\S]*agent\.deleteMember[\s\S]*type:\s*"delete_member"[\s\S]*schema\.renameField[\s\S]*type:\s*"rename_schema_field"[\s\S]*settings\.updateDeployField[\s\S]*type:\s*"update_deploy_settings"[\s\S]*basic\.updateStep[\s\S]*type:\s*"update_flow_step"[\s\S]*graph\.moveNode[\s\S]*type:\s*"move_graph_node"/, "controller plane must translate System, Agent, Schema, Settings, Basic, and Graph UI intents into MobKit authoring operations");
assert.doesNotMatch(controller, /authoringReplacementFromIntent|system\.replaceAuthoringDocument|replace_authoring_document/, "controller plane must not expose whole-document replacement through normal authoring intents");
...
assert.match(app, /<AgentsView[\s\S]*applyAgentIntent=\{applyMobKitAuthoringOperation\}/, "Agent Editor must receive a MobKit authoring operation intent callback from the app shell");
assert.match(agents, /function AddAgentControl\([\s\S]*applyAgentIntent = null[\s\S]*await applyAgentIntent\(\{[\s\S]*intent:\s*"agent\.addDefinition"[\s\S]*definitionId/, "Agent definition adds must call MobKit apply_operation through controller intent translation instead of mutating local member state");
...
assert.match(app, /intent:\s*"system\.reconcileMembers"/, "member reconciliation must execute through a MobKit-owned intent");
assert.match(app, /intent:\s*"system\.reconcileConditionFields"/, "condition-field reconciliation must execute through a MobKit-owned intent");
assert.match(app, /intent:\s*"system\.reconcileContractRefs"/, "contract-ref reconciliation must execute through a MobKit-owned intent");
assert.match(app, /intent:\s*"system\.syncGraphToFlow"[\s\S]*reason:\s*"advanced_graph_changed"/, "advanced graph edits must sync flow through a MobKit-owned intent");
...
assert.match(agents, /applySchemaIntent\(\{\s*intent:\s*"schema\.rename"[\s\S]*schemaId:\s*schema\.id,[\s\S]*newId[\s\S]*\}, result\.selection\)/, "Agent schema rename must submit only schema ids through a controller-translated MobKit intent");
...
assert.match(agentEditorBlock, /applyAgentIntent\(\{[\s\S]*intent:\s*"agent\.deleteMember"[\s\S]*memberId:\s*member\.id[\s\S]*selection/, "Agent delete must submit only member_id through a controller-translated MobKit intent");
assert(!/intent:\s*"agent\.deleteMember"[\s\S]{0,500}(flow|mobSettings|members|instances|edges):/.test(agentEditorBlock), "Agent delete must not submit controller-projected mobpack state");
...
assert.match(mobpackRust, /"type":\s*"connect_graph_nodes"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["from_id",\s*"to_id"\]/, "MobKit operation catalog must advertise graph edge connect as semantic MobKit-owned operation");
assert.match(mobpackRust, /insert_graph_node does not accept raw instance[\s\S]*insert_graph_node does not accept raw instances array[\s\S]*connect_graph_nodes does not accept raw edge/, "MobKit graph authoring must reject raw browser graph rows and edges");
...
assert(!/function studioAddInstancePatch|function studioAppendInstancesPatch/.test(controller), "browser controller must not keep graph node add/append authority");
...
assert(!/function studioAddEdgePatch|function studioAppendEdgesPatch/.test(controller), "browser controller must not keep graph edge add/append authority");
...
assert(!/function graphConnectionAddPatch|function graphConnectionEdgeDraft/.test(controller), "browser controller must not keep graph connection edge draft authority");
...
assert.match(graph, /MobKitFlowController\.graphNodeCanvasState/, "Graph canvas must render node member/tool metadata through the controller plane");
...
assert.match(graph, /MobKitFlowController\.graphCanvasInstances/, "Graph canvas must ask the controller plane for renderable graph instances");
assert.match(graph, /MobKitFlowController\.graphCanvasAdornments/, "Graph canvas must ask the controller plane for non-deployable canvas adornments");
...
assert.match(controller, /function graphSourceFileAdornment/, "controller plane must own Graph source-file adornment projection");
assert.match(controller, /function graphCanvasInstances/, "controller plane must own Graph canvas instance assembly");
assert.match(controller, /function graphCanvasAdornments/, "controller plane must own Graph canvas adornment assembly");
assert.match(controller, /function graphSourceFileAdornmentCanvasState/, "controller plane must own Graph source-file adornment accessibility and display projection");
...
assert(!/\[sourceFileNode,\s*\.\.\.state\.instances\]|graphSourceFileNode\(\{ instances: state\.instances \}\)|graphCanvasInstances\(\{ instances: state\.instances, graphView: canvasView \}\)\.map/.test(graph), "Graph view must not assemble source-file adornment rows into graph instances");
...
assert.match(app, /if \(!result\.changed\) \{[\s\S]{0,200}return;[\s\S]{0,10}\}[\s\S]*applyMobKitAuthoringOperation\(\{[\s\S]*intent:\s*"system\.reconcileContractRefs"/, "app shell must round-trip contract reconciliation changes through a MobKit-owned intent");
assert.match(app, /reconcileShouldRun\("system\.reconcileContractRefs"\)/, "contract reconciliation must pass through the reconcile circuit breaker");
assert(!/intent:\s*"system\.reconcileContractRefs"[\s\S]{0,500}(flow|deploySettings|mobSettings|members|instances|edges):/.test(app), "contract reconciliation must not submit browser-projected document fragments");
...
assert.match(controller, /async function graphProjectionDocument\(document,\s*options = \{\}\)[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("graphProjection"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "controller plane must expose MobKit graph projection through mobkit/mobpacks/graph_projection with AbortSignal out-of-band");
assert.match(controller, /async function graphToFlowDocument\(document,\s*options = \{\}\)[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("graphToFlow"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)[\s\S]*graphToFlow:\s*"mobkit\/mobpacks\/graph_to_flow"/, "controller plane must expose MobKit graph-to-flow projection through mobkit/mobpacks/graph_to_flow with AbortSignal out-of-band");
...
assert(!/MobKitFlowController\.authoringDocumentFromState|const buildAuthoringProjection|const buildDocument/.test(app), "app shell must not construct authoritative mobpack documents from browser editor shards");
...
assert.match(app, /const currentMobKitDocument = \(\) => \{[\s\S]*if \(authoringDocumentRef\.current\) return authoringDocumentRef\.current;[\s\S]*throw new Error\(catalogs\.errorView\.authoringOperationMissingDocumentError/, "API actions must fail closed when there is no MobKit-returned authoring document");
assert.match(app, /const buildMobKitProjectedDocument = async \(overrides = \{\}\) => \{[\s\S]*if \(editorMode !== "advanced"\) \{[\s\S]*try \{[\s\S]*const document = currentMobKitDocument\(\);[\s\S]*return \{ document, requestToken \};[\s\S]*catch \(error\) \{[\s\S]*document: null,[\s\S]*error: error\?\.message \|\| String\(error\)[\s\S]*intent:\s*"system\.syncGraphToFlow"[\s\S]*if \(!result\?\.document\) \{[\s\S]*document:\s*null,[\s\S]*authoringOperationMissingDocumentError[\s\S]*return \{ document: result\.document, requestToken \};/, "Basic and Graph API actions must start from the current MobKit-owned document, with Graph mode failing closed if MobKit sync_graph_to_flow does not return a document");
...
assert.match(builder, /function StepInspector[\s\S]*const applyInputIntent = \(intentRequest\) => \{[\s\S]*return update\(intentRequest\)/, "Basic input-param edits must submit only intent payloads to the controller plane");
assert.match(builder, /intent:\s*"basic\.updateInputParam"[\s\S]*stepId:\s*step\.id,[\s\S]*paramId:\s*id,[\s\S]*patch/, "Basic input-param updates must send explicit intent payloads");
assert.match(builder, /intent:\s*"basic\.renameInputParam"[\s\S]*stepId:\s*step\.id,[\s\S]*paramId:\s*id,[\s\S]*newName:\s*rawName/, "Basic input-param renames must send explicit intent payloads");
assert.match(builder, /intent:\s*"basic\.deleteInputParam"[\s\S]*stepId:\s*step\.id,[\s\S]*paramId:\s*id/, "Basic input-param deletes must send explicit intent payloads");
assert.match(builder, /applyInputIntent\(\{\s*intent:\s*"basic\.addInputParam"[\s\S]*stepId:\s*step\.id/, "Basic input-param adds must send only semantic stepId intent payloads to MobKit");
assert(!/basic\.addInputParam"[\s\S]{0,120}param:/.test(builder), "Basic input-param adds must not send browser-drafted param objects");
assert.match(mobpackRust, /"type":\s*"add_input_param"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["step_id"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise input-param add as a semantic MobKit-owned operation");
...
assert.match(builder, /commitFlow\(\{\s*intent:\s*"basic\.insertStep"[\s\S]*pick,[\s\S]*laneRef/, "Basic editor must send semantic step picks to MobKit insert_flow_step through an intent instead of drafting full steps locally");
assert.match(builder, /commitFlow\(\{\s*intent:\s*"basic\.insertStep"[\s\S]*pick,[\s\S]*laneRef[\s\S]*\}\)\.then\(\(result\) => \{[\s\S]*result\?\.selection\?\.id[\s\S]*setSel\(id\)/, "Basic flow step inserts must apply MobKit-returned selection after the operation projection");
...
assert.match(builder, /commitFlow\(\{\s*intent:\s*"basic\.deleteStep"[\s\S]*stepId:\s*id[\s\S]*\}\)\.then\(\(operationResult\) => \{[\s\S]*operationResult\?\.ok === false[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\)/, "Basic flow step deletes must wait for accepted MobKit results before applying UI-only selection and picker transitions");
assert.match(builder, /intent:\s*"basic\.updateStep"[\s\S]*stepId:\s*requestOrId,[\s\S]*patch/, "Complex Basic flow step branch updates may still send stepId and patch intent payloads to MobKit");
...
assert.match(mobpackRust, /"type":\s*"insert_flow_step"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["step_or_pick",\s*"lane_ref"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise Basic flow-step insert as semantic MobKit-owned");
...
assert.match(builderStepInspectorBlock, /const \[paramAddResult,\s*setParamAddResult\] = React\.useState\(null\);[\s\S]*MobKitFlowController\.inputParamAddErrorState\(paramAddResult,\s*viewState\.authoringOperationFallbackError\)/, "Basic input panel must ask the controller plane to project rejected param-add errors with schema-backed fallback copy");
assert.match(builderStepInspectorBlock, /applyInputIntent\(\{\s*intent:\s*"basic\.addInputParam"[\s\S]*stepId:\s*step\.id[\s\S]*\}\)\.then\(\(result\) => \{[\s\S]*result\?\.ok === false[\s\S]*setParamAddResult\(result\)[\s\S]*setParamAddResult\(null\)/, "Basic input-param add must render rejected MobKit operation results instead of silently dropping them");
...
assert.match(mobpackRust, /fn normalize_input_param_update_patch[\s\S]*unsupported update_input_param patch field/, "MobKit update_input_param must server-normalize and reject raw input-param patch escape hatches");
assert.match(mobpackRust, /"patch_fields":\s*\[[^\]]*"name"[\s\S]*"enumValues"/, "MobKit update_input_param operation metadata must declare its supported patch fields");
...
assert.match(app, /reconcileConditionFieldAvailability\(\{[\s\S]*const flowChanged = result\.flow !== flow;[\s\S]*const edgesChanged = result\.edges !== studio\.edges;[\s\S]*if \(!flowChanged && !edgesChanged\) \{[\s\S]{0,200}return;[\s\S]{0,10}\}[\s\S]*applyMobKitAuthoringOperation\(\{[\s\S]*intent:\s*"system\.reconcileConditionFields"/, "schema-backed condition availability rewrites must round-trip Basic/Graph cleanup through a MobKit-owned intent");
assert(!/intent:\s*"system\.reconcileConditionFields"[\s\S]{0,260}(flow:\s*result\.flow|edges:\s*result\.edges|studio:\s*\{)/.test(app), "condition-field reconciliation must not submit browser-projected flow or edge fragments");
...
assert.match(agents, /const renameField = \(fieldId, oldName, newName\) => \{[\s\S]*applySchemaIntent\(\{\s*intent:\s*"schema\.renameField"[\s\S]*schemaId:\s*schema\.id,[\s\S]*fieldId,[\s\S]*newName/, "Agent Editor schema-field rename commits must send only ids and the new field name to MobKit");
...
assert.match(mobpackRust, /fn normalize_schema_update_patch[\s\S]*unsupported update_schema patch field/, "MobKit update_schema must reject raw schema patch escape hatches");
assert.match(mobpackRust, /fn normalize_schema_field_update_patch[\s\S]*unsupported update_schema_field patch field/, "MobKit update_schema_field must reject raw schema-field patch escape hatches");
...
assert.match(agents, /function SchemaEditor[\s\S]*const deleteSchema = async \(\) => \{[\s\S]*await applySchemaIntent\(\{\s*intent:\s*"schema\.delete"[\s\S]*schemaId:\s*schema\.id[\s\S]*\}, selection\)[\s\S]*onClick=\{deleteSchema\}/, "Agent Editor schema deletes must send only schema_id through MobKit");
...
assert(!/"type":\s*"update_graph_edge"[\s\S]{0,160}"plane":\s*"graph"|"update_graph_edge"\s*=>|graph\.updateEdge/.test(mobpackRust + "\n" + controller + "\n" + app), "Graph edge authoring must not expose raw patch updates; use apply_graph_edge_edit");
...
assert.match(app, /result\.mobSettings !== mobSettings[\s\S]*applyMobKitAuthoringOperation\(\{[\s\S]*intent:\s*"system\.reconcileMembers"/, "app shell must round-trip mob-level profile setting reconciliation through MobKit-owned member reconciliation");
assert(!/intent:\s*"system\.reconcileMembers"[\s\S]{0,500}mobSettings:\s*result\.mobSettings/.test(app), "member reconciliation must not submit browser-projected mob settings");
...
assert.match(controller, /function newFlowInitialState[\s\S]*blankTemplate\.id/, "controller plane must seed new-flow initial template from the MobKit blank mobpack catalog");
...
assert.doesNotMatch(controller, /function newFlowModalStepPatch/, "the new-mob modal is single-step; the step patch must stay retired");
assert.match(controller, /function newFlowModalCreateSpec/, "controller plane must own new-flow create spec projection");
assert.match(controller, /createDisabled:\s*!name\.trim\(\) \|\| !selectedTemplate \|\| !!selectedTemplate\.disabled/, "New mob creation must be disabled until the controller projection has a name and a selected MobKit template row");
...
assert.doesNotMatch(newFlowModalBlock, /trigger/i, "New mob modal must not surface the retired trigger field");
...
assert.match(newFlowModalBlock, /MobKitFlowController\.newFlowModalFieldPatch/, "New mob modal field changes must route through controller state patches");
...
assert.match(controller, /eyebrow:\s*view\.eyebrowTemplate/, "controller plane must project the New Mob modal title from MobKit view chrome");
...
assert.match(app, /MobKitFlowController\.flowRegistryViewState\(flows, currentFlowId, \{\s*canCreate,\s*flowRegistryView,\s*nowUnixMs: Date\.now\(\),\s*\}\)/, "Library view must render controller-projected registry state with MobKit view chrome and a caller-supplied clock");
...
assert.match(app, /inlineSourceToggleTransition\(\{[\s\S]*open:\s*inlineSourceOpen,[\s\S]*currentSurface:\s*inlineSourceSurface,[\s\S]*targetSurface:\s*surface/, "inline source trigger must use controller-projected toggle state instead of always reopening source");
assert.match(app, /inlineSourceToggleButtonState\(\{[\s\S]*open:\s*inlineSourceOpen,[\s\S]*currentSurface:\s*inlineSourceSurface,[\s\S]*targetSurface:\s*"basic"[\s\S]*basicView:\s*catalogs\.basicView,[\s\S]*sourceView:\s*catalogs\.sourceView/, "Basic source button label must come from controller-projected toggle state");
assert.match(app, /<BuilderView[\s\S]*onShowSource=\{\(\) => handleInlineSource\("basic"\)\}[\s\S]*sourceOpen=\{inlineSourceOpen && inlineSourceSurface === "basic"\}[\s\S]*sourceDocument=\{inlineSourceDocument\}[\s\S]*sourceToggleLabel=\{basicSourceToggle\.label\}[\s\S]*sourceView=\{catalogs\.sourceView\}/, "Basic editor mob.toml tile must open and toggle a Basic-scoped inline source-preview-backed source editor");
...
assert.match(builder, /bld-toml-toggle[\s\S]*sourceToggleLabel \|\| viewState\.sourceToggleLabel/, "Basic source-file tile label must render through controller-projected toggle state");
assert(!/\{"\{ \} mob\.toml"\}|\{ \} mob\.toml/.test(builder), "Basic source-file tile must not compose the mob.toml label locally");
assert.match(app, /<GraphEditor[\s\S]*onOpenSourceFile=\{\(sourceRequest\) => handleInlineSource\("graph", sourceRequest\)\}/, "Graph source-file nodes must preserve clicked source identity for the Graph-scoped source-preview-backed inline editor");
...
assert.match(graph, /const selector = canvasView\.sourceFileActivationSelector[\s\S]*closest\?\.\(selector\)[\s\S]*onOpenSourceFile\?\.\(\{[\s\S]*id:\s*sourceEl\.dataset\.sourceId/, "Graph host must delegate source-file activation through schema-backed graph view state");
assert.match(graph, /function SourceFileAdornmentView[\s\S]*<a href=\{adornmentState\.sourceActivationHash\} data-source-id=\{adornment\.id\}[\s\S]*node--source-file/, "Graph mob.toml source file must render as a schema-backed native link adornment, not a graph instance");
assert.match(graph, /className="source-file-adornment node--source-file"/, "Graph mob.toml source file must not inherit graph node chrome");
assert.match(app, /inlineSourceRequestPath\(sourceRequest,[\s\S]*sourceView: catalogs\.sourceView,[\s\S]*graphView: catalogs\.graphView/, "Graph source-file activation must resolve selected archive paths through the controller plane");
assert.match(app, /graphCanvasViewState\(catalogs\.graphView\)[\s\S]*window\.location\.hash !== canvasView\.sourceFileActivationHash[\s\S]*handleInlineSource\("graph", \{ sourcePath: canvasView\.sourcePath \|\| catalogs\.sourceView\.primarySourcePath \}\)/, "Graph mob.toml terminal hash route must open the Graph-scoped inline source editor through MobKit graph-view state");
...
assert.match(graph, /role=\{adornmentState\.role\}[\s\S]*aria-label=\{adornmentState\.ariaLabel\}/, "Graph mob.toml adornment must behave as a controller-projected clickable file affordance");
...
assert.match(controller, /graphSourceFileAdornmentCanvasState\(\{[\s\S]*ariaLabel:\s*view\.sourceFileAriaLabel[\s\S]*sourceActivationHash:\s*view\.sourceFileActivationHash/, "controller plane must project Graph mob.toml adornment accessibility through graph view state");
assert.match(controller, /graphSourceFileAdornment[\s\S]*id:\s*view\.sourceFileNodeId[\s\S]*kind:\s*view\.sourceFileNodeKind[\s\S]*col:\s*minCol \+ view\.sourceFileNodeColOffset[\s\S]*row:\s*minRow \+ view\.sourceFileNodeRowOffset/, "Graph source-file adornment identity and placement must come from MobKit graph view schema");
assert(!/\/mob\\\.toml\/i/.test((controller.match(/function graphNodeCanvasState[\s\S]*?function graphFrameCanvasState/) || [""])[0] + "\n" + (controller.match(/function graphSourceFileAdornment[\s\S]*?function graphGateCanvasState/) || [""])[0]), "Graph source-file projection must not identify source nodes by matching mob.toml text");
...
assert.match(app, /inlineSourceReadyTransition\(\{[\s\S]*\.\.\.nextSourceDocument,[\s\S]*requestedSourcePath \? \{ sourcePath: requestedSourcePath \} : \{\}[\s\S]*\}\)/, "inline source ready state must be controller-projected with controller-selected source path");
...
assert.match(app, /const beginDocumentHydration = React\.useCallback\(\(\) => \{[\s\S]*authoringRevision\.current \+= 1;[\s\S]*clearSourceProjection\(\)/, "flow hydration must invalidate queued authoring work and clear inline source projections before swapping documents");
assert.match(app, /beginDocumentHydration\(\);[\s\S]*setCurrentAuthoringDocument\(hydration\.document\)/, "hydrated documents must advance the authoring epoch before the new document becomes visible to queued operations");
...
assert(!/const buildDocument =|buildAuthoringProjection\(/.test(app), "document construction must not mark browser-built projections as canonical API state");
assert.match(app, /renderCurrentSourceDocument = async \(requestToken,\s*projectedDocument = null\) => \{[\s\S]*await buildMobKitProjectedDocument\(\)[\s\S]*MobKitFlowController\.sourceDocument\(document,\s*currentDraftGuard\(\)\)[\s\S]*if \(!sourceProjectionIsCurrent\(requestToken\)\) return null;[\s\S]*setValidationResults\(projection\.validationRows\)/, "stale or stale-draft mobkit/mobpacks/source responses must not write source or validation state after an edit");
...
assert.match(app, /const handleValidate = async \(\) => \{[\s\S]*const projected = await buildMobKitProjectedDocument\(\);[\s\S]*const document = projected\.document;[\s\S]*requestToken = projected\.requestToken;[\s\S]*MobKitFlowController\.validateDocument\(document,\s*currentDraftGuard\(\)\)/, "MobKit API action freshness tokens must be captured after controller-canonical document projection and bound to the current MobKit draft revision");
assert.match(controller, /async function validateDocument\(document, options = \{\}\) \{[\s\S]*const \{ signal, rkatValidate, rkat_validate,[\s\S]*requestOptions[\s\S]*callRenderRpc\(rpcMethod\("validate"\), \{[\s\S]*rkat_validate:\s*rkatValidate \?\? rkat_validate \?\? true[\s\S]*\}, requestOptions, signal\)/, "Flow Editor validation must request real rkat mob validate evidence by default while passing draft guards through JSON-RPC params and AbortSignal out-of-band");
// Read-only renders carry the optimistic draft guard of the row the UI last
// saw; when our own autosave has already bumped the store the server refuses
// the render as a draft revision conflict. The authoring runner retries that
// race once without the guard; renders must do the same (#398: the Graph-mode
// inline source panel stayed empty behind a "Source render failed" sheet), and
// only that: guard present, guard's refusal, one retry, same document.
assert.match(controller, /async function callRenderRpc\(method, params, requestOptions, signal\) \{[\s\S]*return await callRpc\(method, \{ \.\.\.params, \.\.\.guardedOptions \}, \{ signal \}\);[\s\S]*if \(!hasDraftGuard\(guardedOptions\) \|\| !isDraftGuardConflictError\(error\)\) throw error;[\s\S]*return callRpc\(method, \{ \.\.\.params, \.\.\.unguardedOptions \}, \{ signal \}\);/, "read-only render RPC wrappers must retry a draft-guard conflict exactly once without the store guard and otherwise surface the refusal");
assert.match(controller, /async function deployDocument\(document, options = \{\}\) \{[\s\S]*if \(requestOptions\.execute === false\) \{[\s\S]*return callRenderRpc\(rpcMethod\("deploy"\), \{ document \}, requestOptions, signal\);[\s\S]*\}[\s\S]*return callRpc\(rpcMethod\("deploy"\), \{ document, \.\.\.requestOptions \}, \{ signal \}\);/, "only the deploy PLAN is a read-only render; a real deploy must keep the draft guard and never retry past a conflict");
assert.match(mobpackRust, /pub fn apply_mobpack_authoring_operation[\s\S]*let mut validation = validate_document\(&document\);[\s\S]*validate_with_rkat_requested\(params\) && validation\.ok[\s\S]*validation = validate_document_with_rkat\(params,\s*&document,\s*validation\)\?;/, "MobKit apply_operation must optionally validate operation-mutated documents through rkat mob validate");
assert.match(mobpackRust, /fn validate_document_with_rkat[\s\S]*"validate"\.to_string\(\),[\s\S]*"--trust-policy"\.to_string\(\),[\s\S]*effective_deploy_trust_policy\(&document\.deploy\)/, "rkat mob validate must verify packs under the document's effective deploy trust policy (rkat 0.7 defaults mob validate to strict, rejecting every unsigned editor pack)");
assert.match(mobpackRust, /fn deploy_argv\([\s\S]*"--trust-policy"\.to_string\(\),[\s\S]*effective_deploy_trust_policy\(deploy\)/, "rkat mob run must always execute under the document's effective deploy trust policy so the run matches what SETTINGS shows");
assert.match(mobpackRust, /"trust_policy": DEFAULT_DEPLOY_TRUST_POLICY/, "the deploy-settings schema must advertise the same default trust policy the executed command falls back to");
assert.match(mobpackRust, /const DEFAULT_DEPLOY_TRUST_POLICY: &str = "permissive";/, "the effective deploy trust policy fallback must stay permissive — rkat 0.7's strict default rejects every unsigned editor pack");
assert.match(controller, /async function sourceDocument\(document, options = \{\}\) \{[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRenderRpc\(rpcMethod\("source"\),\s*\{ document \},\s*requestOptions,\s*signal\)/, "source RPC wrapper must pass MobKit draft guards through params and AbortSignal out-of-band (via the read-only render retry seam)");
assert.match(controller, /async function exportDocument\(document, options = \{\}\) \{[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("export"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "export RPC wrapper must pass MobKit draft guards through params and AbortSignal out-of-band");
...
assert.match(controller, /function flowRegistryPersistDocumentProjection\(rows, options = \{\}\)[\s\S]*flowRegistryRowIsRuntimeProjection\(currentRow\)[\s\S]*runtime_projection_read_only[\s\S]*rowPatch: null/, "controller autosave projection must not persist runtime flow projections as local drafts");
assert(!/function flowCatalogBootstrapState[\s\S]{0,600}blankMobpackFromCatalogs|function flowCatalogBootstrapState[\s\S]{0,700}blankFlow|function flowCatalogBootstrapState[\s\S]{0,800}registryFlows\.length \? registryFlows :/.test(controller), "controller bootstrap must not fall back to blank/sample catalog documents as active saved rows");
...
assert.match(controller, /async function callRpc\(method,\s*params,\s*options = \{\}\)[\s\S]*signal:\s*options\.signal/, "controller RPC plane must accept AbortSignal for stale MobKit requests");
assert.match(controller, /async function deployDocument\(document,\s*options = \{\}\)[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("deploy"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "deploy RPC wrapper must pass AbortSignal out-of-band instead of serializing it into MobKit params");
...
assert.match(mobpackRust, /fn default_mobpack_draft_store_path\(\)[\s\S]*storage_layout::default_gateway_home\(\)/, "MobKit draft registry default must derive from the storage layout authority (not private ambient env reads)");
assert.match(mobpackRust, /fn default_mobpack_draft_store_path\(\)[\s\S]*std::env::temp_dir/, "MobKit draft registry must keep the temp-dir last resort when no durable home resolves");
...
assert.match(app, /saveRegistryDocument[\s\S]*MobKitFlowController\.saveDocument\(rowPatch\)[\s\S]*flowRegistryUpsertRowPatch\(rows,\s*result\.row\)[\s\S]*showAuthoringFailure\(error,\s*authoringFailureHead\("draft_save"\)\)/, "app autosave must fold MobKit-returned draft revisions back into registry state and surface schema-backed conflicts");
assert.match(app, /const abort = new AbortController\(\);[\s\S]*loadSchema\(rpcOptions\)[\s\S]*loadCapabilities\(rpcOptions\)[\s\S]*loadCatalogs\(rpcOptions\)[\s\S]*abort\.abort\(\)/, "app bootstrap must abort stale MobKit schema/capability/catalog requests on unmount");
...
assert(!/normalizeBudgetSplitPolicy|budgetSplitPolicyOptions|launchModeBudgetPatch|launchBudgetKindPatch|launchBudgetFixedLimitPatch|memberBudgetAffordanceState/.test(controller), "Flow Editor controller must not retain removed budget-split authoring helpers");
assert.match(controller, /\["budgetSplitPolicy",\s*"budget_split_policy",\s*"budget"\][\s\S]{0,100}return \{\}/, "launch-mode merge patches must reject removed budget-split keys");
...
assert(!/budgetSplitPolicy|budgetPolicyLabel|fixedBudget|set_launch_budget/.test(builder + "\n" + inspector), "Basic/Graph launch controls must not render removed budget-split authoring UI");
...
assert.match(controller, /function launchModeControlState/, "controller plane must own shared launch-control display state");
assert(!/graph_launch_budget_patch/.test(mobpackRust), "MobKit semantic flow-step and graph edits must not retain a budget-split patch implementation");
assert.match(mobpackRust, /set_launch_budget_limit[\s\S]{0,300}expect_err\("removed (?:flow|graph) launch budget action must be rejected"\)/, "MobKit compatibility tests must prove removed launch-budget edits fail closed");
...
assert(!/const\s+defaultLaunchMode|const\s+defaultForkContext/.test(builder + "\n" + inspector), "Basic/Graph UI must not assemble launch defaults directly");
assert(!/label=["'](?:Launch mode|Bridge session|Fork from|Fork context)["']|>LAUNCH MODE · this position<\/div>|>Context<\/label>|placeholder=["']session id["']/.test(builder + "\n" + inspector), "Basic/Graph UI must not compose launch-control labels or placeholders locally");
assert(!/launchTitle:\s*["']Launch mode["']|graphLaunchTitle:\s*["']LAUNCH MODE|resumeSessionLabel:\s*["']Bridge session["']|Unsupported by the MobKit launch_modes contract/.test(controller), "controller must not keep launch-control labels, option labels, or unsupported reason copy as local literals");
...
assert.match(flowEditorBinRust, /--allow-host-deploy/, "standalone Flow Editor must expose explicit opt-in host deploy execution for local rkat mob run testing");
...
assert.match(flowEditorHttpRust, /standalone_flow_editor_rpc_executes_host_deploy_when_explicitly_enabled[\s\S]*deploy_execute_allowed:\s*true[\s\S]*"execute": true[\s\S]*flow-editor-rkat-ok/, "Flow Editor RPC must prove explicit opt-in executes rkat mob run");
assert.match(liveRkatE2eTest, /async function validateHostDeployRpc[\s\S]*mobkit\/mobpacks\/deploy[\s\S]*execute:\s*true[\s\S]*rkat mob run executed/, "live rkat e2e must prove Flow Editor deploy RPC execution, not only direct CLI deploy");
...
assert(!/MobKit validators accepted mobpack|Deploy surface|rkat mob (?:deploy|run) <pack\.mobpack>|rkat exit|rkat output/.test(controller), "controller must not synthesize MobKit validation/deploy display rows when API display_rows are missing");
...
assert.match(app, /React\.useEffect\(\(\) => \{[\s\S]*let cancelled = false;[\s\S]*setDeployCommandPreview\(""\);[\s\S]*buildMobKitProjectedDocument\(\)[\s\S]*MobKitFlowController\.deployCommandPreviewForDocument\(document,\s*\{ \.\.\.currentDraftGuard\(\),\s*signal:\s*abort\.signal \}\)/, "deploy command preview must clear stale MobKit RPC output before requesting a draft-guarded document-backed preview");
...
assert.match(controller, /function deployCommandPreviewForDocument[\s\S]*const \{ signal, packPath, prompt: optionPrompt, deploySettings,[\s\S]*requestOptions[\s\S]*if \(String\(packPath \|\| ""\)\.trim\(\)\) request\.pack_path/, "document-backed deploy preview must only include pack_path when a real caller supplies one and must not serialize transport/UI-only options");
assert.match(controller, /function deployCommandPreviewForDocument[\s\S]*callRpc\(rpcMethod\("deployCommand"\),\s*request,\s*\{ signal \}\)/, "document-backed deploy preview must pass AbortSignal out-of-band while draft guards remain in MobKit params");
assert.match(mobpackRust, /fn guarded_document_actions_reject_stale_draft_revision[\s\S]*mobkit\/mobpacks\/validate draft revision conflict[\s\S]*mobkit\/mobpacks\/source draft revision conflict[\s\S]*mobkit\/mobpacks\/graph_projection draft revision conflict[\s\S]*mobkit\/mobpacks\/graph_to_flow draft revision conflict[\s\S]*mobkit\/mobpacks\/export draft revision conflict[\s\S]*mobkit\/mobpacks\/deploy_command draft revision conflict[\s\S]*mobkit\/mobpacks\/deploy draft revision conflict/, "MobKit tests must prove guarded validate/source/graph/export/deploy-command/deploy requests reject stale draft revisions");
...
assert.match(controller, /runtimeSectionTitle:\s*String\(view\.runtime_section_title/, "controller plane must hydrate Agent runtime section title from MobKit schema");
assert.match(agentEditorBlock, /<details className="section agent-runtime">[\s\S]*editorState\.runtimeSectionTitle[\s\S]*editorState\.profileBindingLabel[\s\S]*editorState\.runtimeModeLabel[\s\S]*<ProviderParamsEditor/, "Agent runtime/profile/backend/provider controls must be demoted behind a schema-backed advanced section");
assert.match(mobpackRust, /required_definition_string\(definition,\s*"model"\)/, "MobKit agent-definition operation must reject API-backed agent definitions without a model");
assert(!/memberFromAgentDefinition[\s\S]{0,900}model:\s*source\.model \|\| ""/.test(controller), "browser controller must not repair missing agent-definition models into empty strings");
assert.match(mobpackRust, /required_definition_string\(definition,\s*"id"\)[\s\S]*required_definition_string\(definition,\s*"sourceMobpack"\)[\s\S]*required_definition_string\(definition,\s*"sourceOrigin"\)/, "MobKit agent-definition operation must reject definitions without schema-provided identity and source provenance");
assert(!/memberFromAgentDefinition[\s\S]{0,1200}slug\(source\.role \|\| source\.name \|\| ["']member["']|memberFromAgentDefinition[\s\S]{0,1200}uniqueMemberName\(source\.name \|\| source\.label \|\| baseRole/.test(controller), "browser controller must not repair missing agent-definition role/name into generic members");
...
assert.match(agents, /applyAgentIntent\(\{[\s\S]*intent:\s*"agent\.addDefinition"/, "Agent Editor must add API-backed agent definitions through the MobKit authoring operation intent API");
assert.doesNotMatch(controller, /function agentDefinitionAddPatch|function agentDefinitionAddByIdPatch|function memberFromAgentDefinition/, "browser controller must not own agent-definition member or schema mutation semantics");
...
assert.match(controller, /function operationErrorText\(result = null, fallback = ""\) \{[\s\S]*if \(!result \|\| typeof result !== "object"\) return ""[\s\S]*return result\.ok === false \? String\(fallback \|\| ""\)\.trim\(\) : ""/, "controller error projections must not show fallback operation errors before an operation has failed");
...
assert.match(agentsListBlock, /const \[schemaAddResult,\s*setSchemaAddResult\] = React\.useState\(null\);[\s\S]*MobKitFlowController\.schemaDefinitionAddErrorState\(schemaAddResult,\s*listState\.schemaAddFallbackError\)/, "Agent schema list must ask the controller plane to project rejected schema-add errors with schema-backed fallback copy");
assert.match(agentsListBlock, /intent:\s*"schema\.add"[\s\S]*result\?\.ok === false[\s\S]*setSchemaAddResult\(result\)[\s\S]*setSchemaAddResult\(null\)[\s\S]*schemaAddErrorState\.hasError[\s\S]*schemaAddErrorState\.text/, "Agent schema add must render rejected MobKit results instead of silently dropping them");
...
assert.match(mobpackRust, /"add_agent_definition"\s*=>\s*\{[\s\S]*apply_add_agent_definition_operation\(&mut document,\s*operation,\s*runtime\)/, "MobKit apply_operation must own agent-definition member and schema mutation semantics through the runtime-backed catalog");
assert.match(mobpackRust, /apply_add_agent_definition_operation[\s\S]*mobpack_agent_definition_by_id\(definition_id,\s*runtime\)[\s\S]*member_from_agent_definition\(&definition,\s*&document\.members\)[\s\S]*merge_agent_definition_schema\(document,\s*&definition\)/, "MobKit agent-definition add must resolve ids, merge bundled schemas, and create members on the MobKit side");
assert.match(mobpackRust, /\[profiles\.agent\][\s\S]*peer_description = "Start from a minimal editable MobKit profile member[\s\S]*fn apply_operation_adds_blank_authoring_profile_member/, "MobKit must expose a backend-owned blank authoring profile member and prove it materializes through add_agent_definition");
...
assert.match(agents, /intent:\s*"schema\.add"[\s\S]*result\?\.selection[\s\S]*setAgentSel\(selection\)/, "Agent schema add must send a semantic MobKit intent and apply MobKit-returned selection");
assert.match(mobpackRust, /"type":\s*"add_schema"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\[\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise schema add as a semantic MobKit-owned operation");
...
assert.match(schemaEditorBlock, /const \[fieldAddResult,\s*setFieldAddResult\] = React\.useState\(null\);[\s\S]*MobKitFlowController\.schemaFieldAddErrorState\(fieldAddResult,\s*schemaState\.fieldAddFallbackError\)/, "Schema editor must ask the controller plane to project rejected field-add errors with schema-backed fallback copy");
assert.match(schemaEditorBlock, /intent:\s*"schema\.addField"[\s\S]*schemaId:\s*schema\.id[\s\S]*result\?\.ok === false[\s\S]*setFieldAddResult\(result\)/, "Schema field add must send a semantic MobKit intent and render rejected MobKit results");
assert.match(mobpackRust, /"type":\s*"add_schema_field"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["schema_id"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise schema-field add as a semantic MobKit-owned operation");
...
assert.match(agents, /const deleteField = \(fieldId\) => \{[\s\S]*applySchemaIntent\(\{\s*intent:\s*"schema\.deleteField"[\s\S]*schemaId:\s*schema\.id,[\s\S]*fieldId/, "Agent Editor schema-field deletes must send only schema_id and field_id through MobKit");
...
assert.match(agents, /intent:\s*"agent\.addTool"[\s\S]*memberId:\s*member\.id[\s\S]*toolId/, "Agent Editor must add tool access through MobKit apply_operation");
assert.match(agents, /intent:\s*"agent\.removeTool"[\s\S]*memberId:\s*member\.id[\s\S]*toolId/, "Agent Editor must remove tool access through MobKit apply_operation");
...
assert.match(mobpackRust, /"add_member_tool"\s*=>\s*apply_member_tool_operation\(&mut document,\s*operation,\s*true,\s*runtime\)/, "MobKit apply_operation must own runtime-catalog-backed tool-add mutations");
assert.match(mobpackRust, /"remove_member_tool"\s*=>\s*\{[\s\S]*apply_member_tool_operation\(&mut document,\s*operation,\s*false,\s*runtime\)/, "MobKit apply_operation must own runtime-catalog-backed tool-remove mutations");
...
assert.match(agents, /intent:\s*"agent\.toggleSkill"[\s\S]*skillId:\s*sid/, "Agent Editor must toggle skill access through MobKit apply_operation");
assert.match(agents, /intent:\s*"agent\.removeSkill"[\s\S]*skillId:\s*sid/, "Agent Editor must remove skill refs through MobKit apply_operation");
assert.match(agents, /intent:\s*"agent\.createInlineSkill"[\s\S]*label:\s*inlineLabel[\s\S]*content:\s*inlineContent/, "Agent Editor must create inline skill definitions and member bindings through MobKit apply_operation");
...
assert.match(mobpackRust, /"toggle_member_skill"\s*=>\s*\{[\s\S]*apply_member_skill_operation\(&mut document,\s*operation,\s*SkillOperation::Toggle,\s*runtime\)/, "MobKit apply_operation must own runtime-catalog-backed skill toggle mutations");
assert.match(mobpackRust, /fn ensure_skill_definition_in_document\([\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*mobpack_catalogs_response_with_runtime\(runtime\)/, "MobKit skill mutations must resolve skill definitions through the runtime-backed catalog when available");
...
assert.match(controller, /function inlineSkillRealmIdFromOperationResult\(result\)[\s\S]*result\?\.selection\?\.skill_id \|\| result\?\.skill_id[\s\S]*result\?\.document\?\.skill_realms/, "controller plane must select the created realm from MobKit-returned operation state");
assert.match(agentsSkillAccessBlock, /MobKitFlowController\.inlineSkillRealmIdFromOperationResult\(result\)[\s\S]*setInlineOpen\(false\)/, "Agent inline skill creation must reset the form after the MobKit operation succeeds");
...
assert.match(mobpackRust, /fn mobpack_agent_definition_by_id\([\s\S]*definition_id: &str,[\s\S]*runtime: Option<&MobpackRuntimeCatalogState>[\s\S]*"agent_definitions"[\s\S]*"sample_agent_definitions"/, "Agent creation must resolve definition ids through runtime-aware MobKit apply_operation");
...
assert.doesNotMatch(controller, /function agentDefinitionAddByIdPatch/, "browser controller must not own agent definition id resolution");
...
assert(!/<div className="section__title">(?:IDENTITY|REALM PROFILE)<\/div>|<span>(?:SYSTEM PROMPT|PEER DESCRIPTION)<\/span>|<label className="field__label">(?:Profile binding|Realm profile|Model|Runtime mode|Backend|Inline peer notifications)<\/label>|placeholder=["'](?:realm profile id|runtime default|Describe the member mandate\. This text is exported as the profile peer_description\.|Describe this member's role for its peers)|title=["'](?:Apply a MobKit profile prompt skeleton["']|Insert the bundled starter template)|>APPLY SKELETON<\/button>|Realm profile refs are import-only|Realm profile refs are exported|This imported member references|This member references|Convert it to an inline profile before validating|Validation\/export preserves the MobKit realm_profile binding/.test(agentEditorBlock), "Agent detail pane must not compose identity/runtime/prompt labels, placeholders, or realm-profile copy locally");
...
assert.match(agentEditorBlock, /const changeSchema = \(rawSchema\) => \{[\s\S]*applyAgentIntent\(\{[\s\S]*intent:\s*"agent\.assignSchema"[\s\S]*memberId:\s*member\.id,[\s\S]*schemaId:\s*rawSchema[\s\S]*onChange=\{e => changeSchema\(e\.target\.value\)\}/, "Agent schema writes must submit only member_id and schema_id through a typed MobKit intent");
assert(!/intent:\s*"agent\.assignSchema"[\s\S]{0,500}(flow|members|instances|edges):/.test(agentEditorBlock), "Agent schema writes must not submit browser-projected Basic or Graph state");
...
assert.match(mobpackRust, /"sourceDefinition":\s*\{[\s\S]*"sourceMobpackName"[\s\S]*"schemaSourceDocumentPath"[\s\S]*"toolDefinitions"[\s\S]*"skillDefinitions"/, "MobKit agent-definition member creation must retain resolved MobKit provenance rows in sourceDefinition");
...
assert(!/budgetTitle|budgetDisabledReason|budgetWeightLabel|budgetTokenCapLabel|budgetSplitPolicyLabels|budget_split_policies/.test(controller), "Agent controller state must not retain removed budget-split affordances or schema fields");
...
assert.match(mobpackRust, /let success = execute\s*&& status_code == Some\(0\)\s*&& run_envelope\s*\.as_ref\(\)\s*\.is_some_and\(\|envelope\| envelope\.status == "completed"\);/, "mobkit/mobpacks/deploy must report success ONLY when rkat mob run executed, exited 0, AND a run envelope parsed as status=completed (exit-0 with an unparseable/missing envelope must NOT be success)");
assert.match(mobpackRust, /if execute \{[\s\S]*std::fs::write\(&pack_path, bytes\)[\s\S]*run_deploy_command/, "mobkit/mobpacks/deploy must write host mobpack archives only when execute=true");
assert.match(mobpackRust, /fn plans_rkat_deploy_without_executing_by_default[\s\S]*assert!\(!result\.executed\);[\s\S]*assert!\(!result\.success\);[\s\S]*assert!\(!std::path::Path::new\(&result\.pack_path\)\.exists\(\)\)/, "MobKit tests must prove deploy plans are read-only and not reported as successful deploys");
assert.match(testSrc("live-rkat-e2e.cjs"), /deploy planning wrote host mobpack without execute=true/, "live e2e must fail if deploy planning writes a host archive without execute=true");
...
assert(!/function uniqueGraphControlSuffix|function uniqueGraphInstanceId|function uniqueGraphEdgeId|function allocateGraphControlCells/.test(controller), "graph quick-insert ID/cell collision handling must live in MobKit operations, not browser controller");
...
assert.match(app, /const buildMobKitProjectedDocument = async[\s\S]*intent:\s*"system\.syncGraphToFlow"[\s\S]*reason:\s*"build_projected_document"/, "Graph-mode export/validate/deploy must reconstruct authoritative flow through MobKit sync_graph_to_flow");
assert.match(app, /handleValidate[\s\S]*buildMobKitProjectedDocument\(\)[\s\S]*validateDocument\(document,\s*currentDraftGuard\(\)\)[\s\S]*handlePublish[\s\S]*buildMobKitProjectedDocument\(\)[\s\S]*exportDocument\(document,\s*currentDraftGuard\(\)\)[\s\S]*handleDeploy[\s\S]*buildMobKitProjectedDocument\(\)[\s\S]*deployDocument\(document,\s*\{ execute,\s*\.\.\.currentDraftGuard\(\) \}/, "validate, export, and deploy must use the MobKit-projected document with current draft guards");
...
assert.doesNotMatch(controller, /function createFlowDraftFromSpec|function flowRegistryCreateDraftProjection|function cloneDocument/, "controller plane must not expose browser-side new-flow draft cloning helpers");
...
assert.match(controller, /function flowDraftIdFromSpec/, "controller plane must own imported-flow ID creation and collision handling");
...
assert.match(mobpackRust, /fn normalize_deploy_settings_field_value[\s\S]*update_deploy_settings surface/, "MobKit deploy settings operations must server-normalize typed enum fields before mutating the document");
assert.match(mobpackRust, /fn normalize_deploy_settings_field_value[\s\S]*update_deploy_settings max_total_tokens/, "MobKit deploy settings operations must server-normalize typed numeric fields before mutating the document");
assert.match(mobpackRust, /fn normalize_deploy_settings_field_value[\s\S]*unknown deploy model/, "MobKit deploy settings operations must reject non-catalog model values before mutating the document");
assert.match(testSrc("live-rkat-e2e.cjs"), /invalid deploy settings were not rejected at the mutation boundary/, "live e2e must prove invalid deploy settings fail at the MobKit mutation boundary");
assert.match(tweaksBlock, /intent:\s*"settings\.updateMobField"[\s\S]*field,[\s\S]*value,/, "Tweaks mob settings mutations must round-trip through MobKit field operation intents");
assert.match(tweaksBlock, /intent:\s*"settings\.editRoleWiring"[\s\S]*\.\.\.operation/, "Tweaks role wiring mutations must round-trip through MobKit action operation intents");
assert(!/intent:\s*field === "roleWiring" \? "settings\.updateRoleWiring" : "settings\.updateMob"[\s\S]{0,320}deploySettings:\s*next/.test(tweaksBlock), "Tweaks mob settings must not submit duplicate browser-projected deploy settings fragments");
assert(!/intent:\s*"settings\.updateMob"[\s\S]{0,260}mobSettings:\s*next|intent:\s*"settings\.updateRoleWiring"[\s\S]{0,260}roleWiring:\s*next/.test(tweaksBlock), "Tweaks mob settings must not submit browser-projected settings payloads");
assert.match(mobpackRust, /"type":\s*"update_mob_settings"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["field"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise mob settings as semantic MobKit-owned");
assert.match(mobpackRust, /"type":\s*"update_role_wiring"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["action"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise role wiring as semantic MobKit-owned");
assert.match(mobpackRust, /update_mob_settings does not accept full mob_settings object[\s\S]*update_mob_settings requires field[\s\S]*update_role_wiring does not accept full role_wiring array[\s\S]*update_role_wiring requires action/, "MobKit mob settings operations must reject projected settings and require typed field/action intents");
assert.match(mobpackRust, /fn normalize_mob_settings_field_value[\s\S]*update_mob_settings backendDefault[\s\S]*update_mob_settings advanced must be a JSON object or null/, "MobKit mob settings operations must server-normalize typed field values before mutating the document");
...
assert.match(roleWiringBlock, /onAction && onAction\(\{ action:\s*"add" \}\)/, "role wiring adds must emit typed MobKit action intents");
...
assert(!/launchModeMergePatch\([^)]*,\s*\{\s*(?:sessionId|from|context):|updateLaunchMode/.test(builderStepInspectorBlock + "\n" + inspectorInstanceBlock), "Basic/Graph launch controls must not assemble launch mode patches locally");
...
assert.doesNotMatch(builderMemberStepControlBlock + "\n" + inspectorInstanceBlock, /MobKitFlowController\.launchMode(?:Kind|Session|ForkSource|ForkContext)Patch/, "Basic and Graph launch controls must not compute launch patches locally");
