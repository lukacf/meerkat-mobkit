...
assert.match(app, /const applyMobKitAuthoringReplacement = async \(overrides = \{\}\) => \{[\s\S]*const document = buildDocument\(\);[\s\S]*const replacement = buildAuthoringProjection\(overrides\);[\s\S]*applyAuthoringOperationDocument\(document,\s*\{[\s\S]*type:\s*overrides\.operationType \|\| "replace_authoring_document"[\s\S]*document:\s*replacement\.document/, "projected editor mutations must send the current document plus operation.document payload through MobKit apply_operation");
