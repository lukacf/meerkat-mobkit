...
assert.match(app, /if \(result\.changed && !hydratingDocumentRef\.current\) markDraft\(\);[\s\S]*if \(result\.members !== studio\.members\) studio\.setMembers\(result\.members\)/, "app shell must invalidate stale validation/source state when MobKit contract reconciliation changes the deployable authoring model");
...
assert.match(builder, /commitFlow\(result\.flow,\s*\{\},\s*"insert_flow_step",\s*\{\s*step:\s*newStep,\s*lane_ref:\s*laneRef\s*\}\);[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\)/, "Basic flow step inserts must send step and lane_ref payloads while applying controller-projected selection and picker transitions");
assert.match(builder, /commitFlow\(result\.flow,\s*\{\},\s*"delete_flow_step",\s*\{\s*step_id:\s*id\s*\}\);[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\)/, "Basic flow step deletes must send step_id payloads while applying controller-projected selection and picker transitions");
...
assert.match(app, /result\.mobSettings !== mobSettings[\s\S]*setMobSettings\(result\.mobSettings\)/, "app shell must apply controller-projected mob-level profile setting reconciliation");
...
assert.match(agentEditorBlock, /const changeSchema = \(rawSchema\) => \{[\s\S]*if \(result\.flow !== flow && setFlow\) setFlow\(result\.flow\);[\s\S]*if \(result\.instances !== studio\.instances\) studio\.setInstances\(result\.instances\);[\s\S]*if \(result\.edges !== studio\.edges\) studio\.setEdges\(result\.edges\);[\s\S]*onChange=\{e => changeSchema\(e\.target\.value\)\}/, "Agent schema writes must apply controller-cleaned Basic flow and Graph state");
