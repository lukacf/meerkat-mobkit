...
assert.match(mobpackRust, /pub struct MobpackRuntimeCatalogState \{[\s\S]*runtime_flow_rows: Vec<Value>[\s\S]*runtime_agent_definition_sources: Vec<Value>/, "MobKit runtime catalog state must carry projected runtime flow rows and profile-definition sources for the Flow Editor registry and Agent Editor");
