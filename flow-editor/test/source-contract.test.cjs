...
assert.match(app, /const applyMobKitAuthoringReplacement = async \(overrides = \{\}\) => \{[\s\S]*const operationType = overrides\.operationType \|\| "replace_authoring_document";[\s\S]*const document = buildDocument\(\);[\s\S]*const replacement = buildAuthoringProjection\(overrides\);[\s\S]*applyAuthoringOperationDocument\(document,\s*\{[\s\S]*type:\s*operationType[\s\S]*document:\s*replacement\.document/, "projected editor mutations must send the current document plus operation.document payload through MobKit apply_operation");
...
assert.match(app, /renderCurrentSourceDocument = async \(requestToken,\s*projectedDocument = null\) => \{[\s\S]*const document = projectedDocument \|\| buildDocument\(\);[\s\S]*MobKitFlowController\.sourceDocument\(document\)[\s\S]*if \(!sourceProjectionIsCurrent\(requestToken\)\) return null;[\s\S]*persistCurrentOutcome\(projection\)/, "stale mobkit/mobpacks/source responses must not write source, validation, or flow-row state after an edit");
