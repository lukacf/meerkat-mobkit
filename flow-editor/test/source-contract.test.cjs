...
assert.match(agents, /applySchemaCascade\(\{[\s\S]*\.\.\.result,[\s\S]*edges:\s*studio\.edges,[\s\S]*\}, result\.selection, "rename_schema", \{[\s\S]*schema_id:\s*schema\.id,[\s\S]*new_id:\s*newId/, "Agent schema rename must submit controller-reconciled Basic flow through a typed MobKit replacement");
...
assert.match(agentEditorBlock, /applyAuthoringReplacement\(\{[\s\S]*operationType:\s*"delete_member"[\s\S]*operation:\s*\{\s*member_id:\s*member\.id\s*\}[\s\S]*flow:\s*result\.flow[\s\S]*mobSettings:\s*result\.mobSettings[\s\S]*members:\s*result\.members[\s\S]*instances:\s*result\.instances[\s\S]*edges:\s*result\.edges/, "Agent delete cascade must submit all controller-projected mobpack state through a typed MobKit replacement");
...
assert.match(app, /studioDeleteInstancePatch\(\{[\s\S]*instances:\s*studio\.instances,[\s\S]*edges:\s*studio\.edges[\s\S]*\}, selection\.id\)[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*selection:\s*result\.selection/, "keyboard graph deletes must apply controller-projected selection clearing through MobKit replacement");
assert.match(app, /studioDeleteEdgePatch\(\{ edges:\s*studio\.edges \}, selection\.id\)[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*selection:\s*result\.selection/, "keyboard graph edge deletes must apply controller-projected selection clearing through MobKit replacement");
...
assert.match(app, /if \(!result\.changed\) return;[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*operationType:\s*"replace_authoring_document"[\s\S]*reason:\s*"reconcile_contract_refs"[\s\S]*flow:\s*result\.flow[\s\S]*deploySettings:\s*result\.deploySettings[\s\S]*mobSettings:\s*result\.mobSettings[\s\S]*members:\s*result\.members[\s\S]*instances:\s*result\.instances[\s\S]*edges:\s*result\.edges/, "app shell must round-trip contract reconciliation changes through an explicit MobKit replacement operation");
...
assert.match(app, /MobKitFlowController\.graphToFlowDocument\(projectionDocument\)[\s\S]*authoringProjectionFromOperationResult\(projectionResult[\s\S]*applyAuthoringDocumentProjection\(projection\)/, "Graph-to-Basic live projection must use MobKit graph_to_flow and apply the returned document projection");
...
assert.match(builder, /inputParamUpdateCascadePatch\(\{[\s\S]*flow,[\s\S]*edges:\s*studio\?\.edges \|\| \[\],[\s\S]*members:\s*studio\?\.members \|\| \[\],[\s\S]*instances:\s*studio\?\.instances \|\| \[\],[\s\S]*schemas:\s*studio\?\.schemas \|\| \[\]/, "Basic input-param semantic cascade must pass current Basic flow, Graph edges, and real Agent schemas into controller cleanup");
assert.match(builder, /inputParamRenameCascadePatch\(\{[\s\S]*flow,[\s\S]*edges:\s*studio\?\.edges \|\| \[\][\s\S]*\}, step\.id, id, rawName, previousName, contract\)/, "Basic input-param rename cascade must pass current Basic flow and Graph edges into controller cleanup");
assert.match(builder, /inputParamDeleteCascadePatch\(\{[\s\S]*flow,[\s\S]*edges:\s*studio\?\.edges \|\| \[\][\s\S]*\}, step\.id, id, contract\)/, "Basic input-param delete cascade must pass current Basic flow and Graph edges into controller cleanup");
assert.match(builder, /applyAuthoringReplacement\(\{[\s\S]*operationType,[\s\S]*operation,[\s\S]*flow:\s*result\.flow,[\s\S]*studio:\s*\{\s*edges:\s*result\.edges\s*\}/, "Basic input-param cascades must submit graph condition-edge changes and operation payloads through MobKit replacement");
...
assert.match(builder, /if \(!commitFlow\(result\.flow,\s*\{\},\s*"delete_flow_step",\s*\{\s*step_id:\s*id\s*\}\)\) return;[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\)/, "Basic flow step deletes must send step_id payloads before applying controller-projected selection and picker transitions");
...
assert.match(app, /reconcileConditionFieldAvailability\(\{[\s\S]*const flowChanged = result\.flow !== flow;[\s\S]*const edgesChanged = result\.edges !== studio\.edges;[\s\S]*if \(!flowChanged && !edgesChanged\) return;[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*operationType:\s*"replace_authoring_document"[\s\S]*reason:\s*"reconcile_condition_fields"[\s\S]*flow:\s*result\.flow[\s\S]*edges:\s*result\.edges/, "schema-backed condition availability rewrites must round-trip Basic/Graph cleanup through an explicit MobKit replacement operation");
...
assert.match(agents, /const renameField = \(fieldId, oldName, newName\) => \{[\s\S]*schemaFieldRenameCascadePatch\([\s\S]*applySchemaCascade\(result,[\s\S]*"rename_schema_field",\s*\{[\s\S]*schema_id:\s*schema\.id,[\s\S]*field_id:\s*fieldId,[\s\S]*new_name:\s*newName/, "Agent Editor schema-field rename commits must apply controller-cleaned schemas, Basic flow, Graph edges, and explicit MobKit payloads together");
...
assert.match(agents, /function SchemaEditor[\s\S]*const deleteSchema = \(\) => \{[\s\S]*applySchemaCascade\(result, result\.selection,\s*"delete_schema",\s*\{\s*schema_id:\s*schema\.id\s*\}\);[\s\S]*onClick=\{deleteSchema\}/, "Agent Editor schema deletes must apply controller-cleaned Basic flow and Graph edges with an explicit MobKit schema_id payload");
assert.match(controller, /function studioDeleteSchemaPatch[\s\S]*selection:\s*null/, "controller plane must own schema-delete selection clearing");
assert.match(schemaEditorBlock, /const deleteSchema = \(\) => \{[\s\S]*setAgentSel\(result\.selection\)/, "Agent schema deletes must apply controller-projected selection clearing");
assert(!/const deleteSchema = \(\) => \{[\s\S]*setAgentSel\(null\)/.test(schemaEditorBlock), "Agent schema deletes must not clear selection locally");
...
assert.match(app, /result\.mobSettings !== mobSettings[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*operationType:\s*"replace_authoring_document"[\s\S]*reason:\s*"reconcile_members"[\s\S]*mobSettings:\s*result\.mobSettings/, "app shell must round-trip controller-projected mob-level profile setting reconciliation through MobKit replacement");
...
assert.match(agentsSchemaFieldBlock, /onChange=\{e => setDraftName\(e\.target\.value\)\}[\s\S]*onBlur=\{e => \{[\s\S]*onRename\(previous,\s*normalized\)/, "Agent Editor schema-field names must commit through the controller rename cascade on blur");
...
assert.match(agents, /const deleteField = \(fieldId\) => \{[\s\S]*schemaFieldDeleteCascadePatch\([\s\S]*applySchemaCascade\(result,[\s\S]*"delete_schema_field",\s*\{[\s\S]*schema_id:\s*schema\.id,[\s\S]*field_id:\s*fieldId/, "Agent Editor schema-field deletes must apply controller-cleaned Basic flow and Graph edges with explicit MobKit schema_id and field_id payloads");
...
assert.match(agentEditorBlock, /const changeSchema = \(rawSchema\) => \{[\s\S]*applyAuthoringReplacement\(\{[\s\S]*operationType:\s*"assign_member_schema"[\s\S]*operation:\s*\{\s*member_id:\s*member\.id,\s*schema_id:\s*rawSchema\s*\}[\s\S]*flow:\s*result\.flow[\s\S]*members:\s*result\.members[\s\S]*instances:\s*result\.instances[\s\S]*edges:\s*result\.edges[\s\S]*onChange=\{e => changeSchema\(e\.target\.value\)\}/, "Agent schema writes must submit controller-cleaned Basic flow and Graph state through a typed MobKit replacement");
...
assert.match(agentEditorBlock, /setSchemaChangeResult\(result\);[\s\S]*if \(!result\.ok\) return;[\s\S]*setSchemaChangeResult\(null\);[\s\S]*schemaErrorState\.hasError[\s\S]*schemaErrorState\.text/, "Agent schema writes must render rejected controller results instead of silently dropping them");
...
assert.match(app, /const buildMobKitProjectedDocument = async[\s\S]*graphToFlowDocument\(document\)[\s\S]*authoringProjectionFromOperationResult\(result/, "Graph-mode export/validate/deploy must reconstruct authoritative flow through MobKit graph_to_flow");
...
assert.match(tweaksBlock, /operationType:\s*"update_deploy_settings"[\s\S]*operation:\s*\{\s*deploy:\s*next\s*\}[\s\S]*deploySettings:\s*next/, "Tweaks deploy settings mutations must round-trip through named MobKit operation payloads");
assert.match(tweaksBlock, /operationType:\s*field === "roleWiring" \? "update_role_wiring" : "update_mob_settings"[\s\S]*operation:\s*field === "roleWiring" \? \{\s*role_wiring:\s*next\.roleWiring \|\| \[\]\s*\} : \{\s*mob_settings:\s*next\s*\}[\s\S]*mobSettings:\s*next/, "Tweaks mob settings mutations must round-trip through named MobKit operation payloads");
