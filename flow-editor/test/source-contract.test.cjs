...
assert.match(agents, /MobKitFlowController\.memberUpdateCascadePatch/, "Agent Editor must update agents through a controller-owned mobpack cascade");
assert.match(controller, /function memberUpdateCascadePatch/, "controller plane must own Agent update cascade semantics across Basic, Graph, and mob settings");
assert.match(agentEditorBlock, /memberUpdateCascadePatch\(\{[\s\S]*memberId:\s*member\.id,[\s\S]*members:\s*studio\.members,[\s\S]*flow,[\s\S]*instances:\s*studio\.instances,[\s\S]*edges:\s*studio\.edges,[\s\S]*mobSettings,[\s\S]*contract/, "Agent update cascade must pass Basic, Graph, Agent, mob settings, and contract state to the controller plane");
assert.match(agentEditorBlock, /studio\.setMembers\(result\.members\)[\s\S]*setFlow\(result\.flow\)[\s\S]*studio\.setInstances\(result\.instances\)[\s\S]*studio\.setEdges\(result\.edges\)[\s\S]*setMobSettings\(result\.mobSettings\)/, "Agent update cascade must apply all controller-projected mobpack state together");
...
assert.match(app, /const buildAuthoringProjection = \(\) => window\.MobKitFlowController\.authoringDocumentFromState/, "app shell must expose the controller authoring projection separately from the document");
...
assert.match(builder, /setFlow\(result\.flow\);[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\);/, "Basic flow step insert/delete must apply controller-projected flow, selection, and picker transitions");
...
assert.match(agents, /const renameField = \(fieldId, oldName, newName\) => \{[\s\S]*schemaFieldRenameCascadePatch\([\s\S]*if \(studio\.snap\) studio\.snap\(\);[\s\S]*studio\.setSchemas\(result\.schemas\);[\s\S]*if \(flowChanged && setFlow\) setFlow\(result\.flow\);[\s\S]*if \(edgesChanged\) studio\.setEdges\(result\.edges\);/, "Agent Editor schema-field rename commits must snapshot and apply controller-cleaned schemas, Basic flow, and Graph edges together");
...
assert.match(controller, /memberInlineSkillCascadePatch[\s\S]*inlineForm:\s*\{[\s\S]*realmId:\s*result\.realmId,[\s\S]*open:\s*false/, "controller plane must own Agent inline-skill post-create form transitions");
assert.match(agentsSkillAccessBlock, /const inlineForm = result\.inlineForm \|\| \{\};[\s\S]*setRealmId\(inlineForm\.realmId \|\| result\.realmId\)[\s\S]*setInlineOpen\(\!\!inlineForm\.open\)/, "Agent inline skill creation must apply controller-projected form transitions");
assert(!/setInlineLabel\(""\);\s*setInlineContent\(""\);\s*setInlineError\(""\);\s*setInlineOpen\(false\)/.test(agentsSkillAccessBlock), "Agent inline skill creation must not clear and close the inline form locally");
