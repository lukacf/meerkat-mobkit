...
assert.match(controller, /function flowCatalogBootstrapState\(catalogPayload, options = \{\}\)[\s\S]*const registryFlows = flowRegistryRowsFromBackend\(options\.registryRows \|\| options\.registryResult\?\.rows\);[\s\S]*const runtimeFlows = flowRegistryRowsFromBackend\(catalogPayload\?\.runtime_flows\);[\s\S]*\.\.\.runtimeFlows\.filter/, "controller plane must merge MobKit-saved draft rows with runtime-projected MobKit flow rows during bootstrap");
