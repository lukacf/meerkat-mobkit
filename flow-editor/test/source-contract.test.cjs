...
assert.match(mobpackRust, /"add_agent_definition"\s*=>\s*apply_add_agent_definition_operation/, "MobKit apply_operation must own agent-definition member and schema mutation semantics");
assert.match(mobpackRust, /apply_add_agent_definition_operation[\s\S]*mobpack_agent_definition_by_id\(definition_id\)[\s\S]*member_from_agent_definition\(&definition,\s*&document\.members\)[\s\S]*merge_agent_definition_schema\(document,\s*&definition\)/, "MobKit agent-definition add must resolve ids, merge bundled schemas, and create members on the MobKit side");
