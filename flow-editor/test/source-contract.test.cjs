...
assert.match(agents, /memberToolAccessCascadePatch\(\{[\s\S]*memberId:\s*member\.id,[\s\S]*flow,[\s\S]*instances:\s*studio\.instances,[\s\S]*\}, raw, toolCatalog\)/, "Agent Editor tool-add cascade must pass MobKit catalog and current Basic/Graph projections into controller validation");
...
assert.match(agents, /MobKitFlowController\.memberToolAccessState/, "Agent Editor must render tool access rows through the controller plane");
