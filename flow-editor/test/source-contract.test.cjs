...
assert.match(controller, /const graphProjection = graphProjectionFromMobKitResult\(result\)[\s\S]*graphProjectionForDocument\(\{ \.\.\.document, flow \}, members, options\.contract\)/, "hydration must prefer MobKit graph projection RPC payloads before falling back to local stored graph data");
...
assert.match(app, /graphToFlow\(\{[\s\S]*previousFlow:\s*flow,[\s\S]*contract[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*operationType:\s*"replace_authoring_document"[\s\S]*reason:\s*"project_graph_to_flow"[\s\S]*flow:\s*nextFlow/, "Graph-to-Basic live projection must use the loaded MobKit graph contract and round-trip through MobKit replacement");
...
assert.match(app, /const buildDocument = \(\) => \{[\s\S]*const projection = buildAuthoringProjection\(\);[\s\S]*applyAuthoringDocumentProjection\(projection\);[\s\S]*return projection\.document;/, "current document construction must synchronize UI state with the controller-canonical projection");
...
assert.match(app, /const buildDocument = \(\) => \{[\s\S]*const projection = buildAuthoringProjection\(\);[\s\S]*beginProjectionSync\(\);[\s\S]*applyAuthoringDocumentProjection\(projection\);[\s\S]*return projection\.document;/, "document construction must mark controller projection sync before applying canonical UI state");
assert.match(app, /renderCurrentSourceDocument = async \(requestToken,\s*projectedDocument = null\) => \{[\s\S]*const document = projectedDocument \|\| buildDocument\(\);[\s\S]*MobKitFlowController\.sourceDocument\(document\)[\s\S]*if \(!sourceProjectionIsCurrent\(requestToken\)\) return null;[\s\S]*setValidationResults\(projection\.validationRows\)/, "stale mobkit/mobpacks/source responses must not write source or validation state after an edit");
...
assert.match(app, /const handleValidate = async \(\) => \{[\s\S]*const document = buildDocument\(\);[\s\S]*requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.validateDocument\(document\)/, "MobKit API action freshness tokens must be captured after controller-canonical document projection");
...
assert.match(app, /const handleValidate = async \(\) => \{[\s\S]*const document = buildDocument\(\);[\s\S]*requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.validateDocument\(document\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale validation responses must not write validation, stage, or flow-row state after an edit");
...
assert.match(app, /const handlePublish = async \(\) => \{[\s\S]*const document = buildDocument\(\);[\s\S]*requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.exportDocument\(document\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale publish/export responses must not write published validation state after an edit");
assert.match(app, /const handleDeploy = async \(\{ execute \}\) => \{[\s\S]*const document = buildDocument\(\);[\s\S]*requestToken = currentAuthoringRevision\(\);[\s\S]*MobKitFlowController\.deployDocument\(document, \{ execute \}\);[\s\S]*if \(!authoringRevisionIsCurrent\(requestToken\)\) return;[\s\S]*persistCurrentOutcome\(outcome\)/, "stale deploy responses must not write validation, stage, or flow-row state after an edit");
