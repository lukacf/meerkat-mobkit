...
const applyAuthoringProjectionBlock = (app.match(/const applyAuthoringDocumentProjection = \(projection\) => \{[\s\S]*?const buildAuthoringProjection/) || [""])[0];
...
assert.match(controller, /async function applyAuthoringOperationDocument\(document,\s*operation,\s*options = \{\}\)[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("applyOperation"\),\s*\{ document,\s*operation,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "controller plane must apply editor mutations through mobkit/mobpacks/apply_operation while carrying out-of-band draft guards");
assert.match(controller, /function authoringOperationFromIntent[\s\S]*system\.reconcileMembers[\s\S]*type:\s*"reconcile_members"[\s\S]*agent\.updateMember[\s\S]*type:\s*"update_member"[\s\S]*agent\.deleteMember[\s\S]*type:\s*"delete_member"[\s\S]*schema\.renameField[\s\S]*type:\s*"rename_schema_field"[\s\S]*settings\.updateDeploy[\s\S]*type:\s*"update_deploy_settings"[\s\S]*basic\.updateStep[\s\S]*type:\s*"update_flow_step"[\s\S]*graph\.moveNode[\s\S]*type:\s*"move_graph_node"/, "controller plane must translate System, Agent, Schema, Settings, Basic, and Graph UI intents into MobKit authoring operations");
assert.match(controller, /function authoringReplacementFromIntent[\s\S]*system\.replaceAuthoringDocument[\s\S]*operationType:\s*"replace_authoring_document"/, "controller plane must reserve replacement translation for explicit whole-document replacement");
...
assert.match(app, /const applyMobKitAuthoringOperation = async \(operation\) => \{[\s\S]*authoringOperationFromIntent\(operation\)[\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*currentDraftGuard\(\)\)[\s\S]*authoringProjectionFromOperationResult\(result/, "app shell must round-trip controller-translated authoring mutations through MobKit before applying projected state");
...
assert.match(app, /authoringReplacementFromIntent[\s\S]*const operation = \{[\s\S]*type:\s*operationType[\s\S]*applyAuthoringOperationDocument\(document,[\s\S]*operation[\s\S]*currentDraftGuard\(\)\)/, "projected authoring replacements must carry the same MobKit draft revision guard");
assert.match(app, /useAuthoringProjection[\s\S]*operation\.document = buildAuthoringProjection\(normalizedOverrides\)\.document/, "only controller-marked document replacement intents may send operation.document through MobKit apply_operation");
...
assert.match(app, /intent:\s*"graph\.insertNode"[\s\S]*instance/, "Graph instance adds must send the inserted instance intent payload through the controller plane");
...
assert.match(app, /intent:\s*"graph\.updateNode"[\s\S]*instanceId:\s*id,[\s\S]*patch/, "Graph instance updates must send instanceId and patch through the controller plane");
...
assert.match(app, /intent:\s*"graph\.connectNodes"[\s\S]*operation,/, "Graph edge adds must send a MobKit graph connect intent");
...
assert.match(app, /intent:\s*"graph\.updateEdge"[\s\S]*edgeId:\s*id,[\s\S]*patch/, "Graph edge updates must send edgeId and patch through the controller plane");
...
assert.match(app, /const result = e\.shiftKey \? studio\.redo\(\) : studio\.undo\(\);[\s\S]*applyMobKitAuthoringReplacement\(\{[\s\S]*intent:\s*"system\.replaceAuthoringDocument"[\s\S]*reason:\s*e\.shiftKey \? "redo" : "undo"[\s\S]*studio:\s*result\.state/, "app shell must commit undo/redo transitions through MobKit replacement");
...
assert.match(app, /const buildMobKitProjectedDocument = async \(overrides = \{\}\) => \{[\s\S]*if \(editorMode !== "advanced"\) \{[\s\S]*const document = currentMobKitDocument\(overrides\);[\s\S]*return \{ document, requestToken \};[\s\S]*intent:\s*"system\.syncGraphToFlow"[\s\S]*if \(!result\?\.document\) \{[\s\S]*document:\s*null,[\s\S]*authoringOperationMissingDocumentError[\s\S]*return \{ document: result\.document, requestToken \};/, "Basic and Graph API actions must start from the current MobKit-owned document, with Graph mode failing closed if MobKit sync_graph_to_flow does not return a document");
...
assert.match(app, /const document = currentMobKitDocument\(\);[\s\S]*applyAuthoringOperationDocument\(document,\s*translatedOperation,\s*currentDraftGuard\(\)\)/, "Agent add-control operations must send the full current MobKit-returned deployable document into MobKit validation with draft revision guards");
...
assert.match(mobpackRust, /"type":\s*"update_deploy_settings"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["deploy_or_field"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise deploy settings as semantic MobKit-owned");
assert.match(mobpackRust, /update_deploy_settings requires deploy object or field/, "MobKit deploy settings operation must reject projected fragments without a deploy payload or field intent");
...
assert.match(mobpackRust, /"type":\s*"update_mob_settings"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["mob_settings_or_field"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise mob settings as semantic MobKit-owned");
assert.match(mobpackRust, /"type":\s*"update_role_wiring"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["role_wiring_or_action"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise role wiring as semantic MobKit-owned");
assert.match(mobpackRust, /update_mob_settings requires mob_settings object or field[\s\S]*update_role_wiring requires role_wiring, mob_settings, or action/, "MobKit mob settings operations must reject projected fragments without typed payloads or action intents");
