...
assert.match(controller, /mobkit\/mobpacks\/export did not return source_files/, "Source drawer must fail closed when MobKit export omits archive member metadata");
assert.match(controller, /mobkit\/mobpacks\/export did not return mobkit\/mob\.toml source file/, "Source drawer must fail closed when MobKit export omits the real mob.toml archive member");
...
assert.match(app, /exportCurrentSourceDocument = async \(requestToken,\s*projectedDocument = null\) => \{[\s\S]*const document = projectedDocument \|\| buildDocument\(\);[\s\S]*MobKitFlowController\.exportDocument\(document\)[\s\S]*if \(!sourceProjectionIsCurrent\(requestToken\)\) return null;[\s\S]*persistCurrentOutcome\(projection\)/, "stale mobkit/mobpacks/export responses must not write source, validation, or flow-row state after an edit");
