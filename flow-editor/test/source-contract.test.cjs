...
assert.match(builder, /MobKitFlowController\.flowStepInsertTransition/, "Basic editor must insert flow steps through the controller transition plane");
assert.match(builder, /flowStepInsertTransition\(flow,\s*laneRef,\s*newStep,\s*\{\s*members\s*\}\)/, "Basic flow step inserts must pass real MobKit members into controller validation");
...
assert.match(builder, /if \(!commitFlow\(result\.flow,\s*\{\},\s*"insert_flow_step",\s*\{\s*step:\s*newStep,\s*lane_ref:\s*laneRef\s*\}\)\) return;[\s\S]*setSel\(result\.selection\);[\s\S]*setPicker\(result\.picker\)/, "Basic flow step inserts must send step and lane_ref payloads before applying controller-projected selection and picker transitions");
...
assert.match(mobpackRust, /"type":\s*"insert_flow_step"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["step",\s*"lane_ref"\]/, "MobKit operation catalog must advertise Basic flow-step insert as MobKit-owned");
