...
assert.match(controller, /function authoringOperationFromIntent[\s\S]*agent\.updateMember[\s\S]*type:\s*"update_member"[\s\S]*basic\.updateStep[\s\S]*type:\s*"update_flow_step"[\s\S]*graph\.moveNode[\s\S]*type:\s*"move_graph_node"/, "controller plane must translate Agent, Basic, and Graph UI intents into MobKit authoring operations");
assert.match(controller, /function authoringReplacementFromIntent[\s\S]*settings\.updateDeploy[\s\S]*operationType:\s*"update_deploy_settings"/, "controller plane must translate settings and explicit projection intents into MobKit replacement operations");
