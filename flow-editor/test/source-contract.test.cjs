...
assert.match(app, /if \(!Array\.isArray\(registryPayload\?\.rows\) \|\| registryPayload\.rows\.length === 0\) \{[\s\S]*MobKitFlowController\.createDocument\(\{[\s\S]*template:\s*"blank"[\s\S]*trigger:\s*"MobKit editor startup draft"[\s\S]*\},\s*rpcOptions\)/, "app shell must create a real MobKit blank draft before opening an empty editor registry");
...
assert.match(app, /MobKitFlowController\.flowCatalogBootstrapState\(catalogPayload,\s*\{[\s\S]*deployDefaults:\s*nextCatalogs\.deployDefaults,[\s\S]*mobDefaults:\s*nextCatalogs\.mobDefaults/, "app catalog bootstrap must ask the controller plane for saved registry/template rows and hydration options");
...
assert.match(controller, /function flowCatalogBootstrapState\(catalogPayload, options = \{\}\)[\s\S]*const registryFlows = flowRegistryRowsFromBackend\(options\.registryRows \|\| options\.registryResult\?\.rows\);[\s\S]*const flows = registryFlows;/, "controller plane must open only MobKit-saved registry rows during bootstrap");
