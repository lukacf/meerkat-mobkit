...
assert.match(app, /if \(!result\.changed\) return;[\s\S]*applyMobKitAuthoringOperation\(\{[\s\S]*intent:\s*"system\.reconcileContractRefs"/, "app shell must round-trip contract reconciliation changes through a MobKit-owned intent");
