...
assert.match(builder, /function StepInspector[\s\S]*const applyInputOperation = \(operationType, operation = \{\}\) => \{[\s\S]*applyAuthoringReplacement\(\{ operationType, operation \}\)/, "Basic input-param edits must submit only typed operation payloads to MobKit");
...
assert.match(builder, /operationType:\s*"insert_flow_step"[\s\S]*operation:\s*\{\s*pick,\s*lane_ref:\s*laneRef\s*\}/, "Basic editor must send semantic step picks to MobKit insert_flow_step instead of drafting full steps locally");
assert.match(builder, /applyAuthoringReplacement\(\{[\s\S]*operationType:\s*"insert_flow_step"[\s\S]*\}\)\.then\(\(result\) => \{[\s\S]*result\?\.selection\?\.id[\s\S]*setSel\(id\)/, "Basic flow step inserts must apply MobKit-returned selection after the operation projection");
...
assert.match(agents, /function SchemaEditor[\s\S]*const deleteSchema = \(\) => \{[\s\S]*applySchemaOperation\(selection,\s*"delete_schema",\s*\{\s*schema_id:\s*schema\.id\s*\}\);[\s\S]*onClick=\{deleteSchema\}/, "Agent Editor schema deletes must send only schema_id through MobKit");
assert.match(schemaEditorBlock, /const deleteSchema = \(\) => \{[\s\S]*const selection = \{ kind: null, id: null \};[\s\S]*setAgentSel\(selection\)/, "Agent schema deletes may clear Agent-editor selection as UI state");
