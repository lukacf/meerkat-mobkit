...
assert.match(graph, /const openSourceFromEvent = \(e\) => \{[\s\S]*closest\?\.\(".*node--source-file.*"\)[\s\S]*onOpenSourceFile\?\.\(\{[\s\S]*id:\s*sourceEl\.dataset\.instId/, "Graph host must delegate source-file activation to the app/control boundary");
assert.match(graph, /<a href="#mobkit-graph-source" data-inst-id=\{inst\.id\}[\s\S]*node--source-file/, "Graph mob.toml terminal must use a native link activation surface while preserving source-file node styling");
assert.match(app, /window\.location\.hash !== "#mobkit-graph-source"[\s\S]*handleInlineSource\("graph"\)/, "Graph mob.toml terminal hash route must open the Graph-scoped inline source editor through the app boundary");
