...
assert.match(app, /const applyAuthoringDocumentProjection = \(projection\) => \{[\s\S]*graphStructureSignature\(projection\.instances,\s*projection\.edges \|\| \[\],\s*\{\s*members:\s*projection\.members \|\| studio\.members,\s*contract\s*\}\)[\s\S]*studio\.setInstances\(projection\.instances \|\| \[\]\)[\s\S]*studio\.setEdges\(projection\.edges \|\| \[\]\)[\s\S]*studio\.setFrames\(projection\.frames \|\| \[\]\)/, "app shell must apply controller-canonical Graph projection before using the deployable document without treating layout reconciliation as an edit");
