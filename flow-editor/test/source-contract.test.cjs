...
assert.match(tweaksBlock, /intent:\s*"settings\.updateDeploy"[\s\S]*deploy:\s*next/, "Tweaks deploy settings mutations must round-trip through named MobKit operation intents");
assert(!/intent:\s*"settings\.updateDeploy"[\s\S]{0,220}deploySettings:\s*next/.test(tweaksBlock), "Tweaks deploy settings must not submit duplicate browser-projected settings fragments");
assert.match(mobpackRust, /"type":\s*"update_deploy_settings"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["deploy"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise deploy settings as semantic MobKit-owned");
...
assert.match(tweaksBlock, /intent:\s*field === "roleWiring" \? "settings\.updateRoleWiring" : "settings\.updateMob"[\s\S]*roleWiring:\s*next\.roleWiring \|\| \[\],[\s\S]*mobSettings:\s*next/, "Tweaks mob settings mutations must round-trip through named MobKit operation intents");
...
assert.match(mobpackRust, /"type":\s*"update_mob_settings"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["mob_settings"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise mob settings as semantic MobKit-owned");
assert.match(mobpackRust, /"type":\s*"update_role_wiring"[\s\S]*"authority":\s*"mobkit"[\s\S]*"requires":\s*\["role_wiring"\][\s\S]*"projection_document_supported":\s*false/, "MobKit operation catalog must advertise role wiring as semantic MobKit-owned");
