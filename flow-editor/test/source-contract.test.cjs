...
const sourceProjectionBlock = (app.match(/const exportCurrentSourceDocument = async[\s\S]*?const handleSource = async/) || [""])[0];
...
assert.match(controller, /mobkit\/mobpacks\/source[\s\S]*mobkit\/mobpacks\/export/, "Source drawer projection must accept MobKit source-preview and export result metadata");
assert.match(controller, /did not return source_files/, "Source drawer must fail closed when MobKit source/export omits archive member metadata");
assert.match(controller, /did not return mobkit\/mob\.toml source file/, "Source drawer must fail closed when MobKit source/export omits the real mob.toml archive member");
...
assert.match(app, /MobKitFlowController\.sourceDocumentFromExport\(document, result, \{\s*sourceView:\s*catalogs\.sourceView,\s*\}\)/, "app shell must project MobKit source/export results through the controller plane with the schema-backed source-view contract");
...
assert.match(app, /const handleValidate = async \(\) => \{[\s\S]*finally \{[\s\S]*if \(requestToken === null \|\| authoringRevisionIsCurrent\(requestToken\)\) \{[\s\S]*setValidate\(true\);[\s\S]*\}[\s\S]*setApiBusy\(false\);[\s\S]*\}/, "stale validation responses must not open the validation sheet after an edit");
