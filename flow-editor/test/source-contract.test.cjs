...
assert.match(controller, /async function validateDocument\(document, options = \{\}\) \{[\s\S]*const \{ signal, rkatValidate, rkat_validate,[\s\S]*requestOptions[\s\S]*rkat_validate:\s*rkatValidate \?\? rkat_validate \?\? true[\s\S]*\.\.\.requestOptions[\s\S]*\{ signal \}/, "Flow Editor validation must request real rkat mob validate evidence by default while passing draft guards through JSON-RPC params and AbortSignal out-of-band");
...
assert.match(controller, /async function sourceDocument\(document, options = \{\}\) \{[\s\S]*const \{ signal,[\s\S]*requestOptions[\s\S]*callRpc\(rpcMethod\("source"\),\s*\{ document,\s*\.\.\.requestOptions \},\s*\{ signal \}\)/, "source RPC wrapper must pass MobKit draft guards through params and AbortSignal out-of-band");
