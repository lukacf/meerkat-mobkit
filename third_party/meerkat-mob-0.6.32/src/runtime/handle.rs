...
pub struct MemberDeliveryReceipt {
    ...
    /// Completed run result when the delivery path can observe terminal
    /// completion. Local admission-only deliveries leave this empty and rely
    /// on normal session event/history projection.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl MobHandle {
    ...
    async fn execute_machine_command(
        ...
        match command {
            ...
            MobMachineCommand::SubmitWork(cmd) => {
                ...
                let completion = self
                    .send_actor_command(|reply_tx| MobCommand::SubmitWork { payload, reply_tx })
                    .await??;
    ...
    pub(super) async fn external_turn_for_member(
        ...
        let completion = match self
            ...
            .await?
        {
            MobMachineCommandResult::WorkReceipt { completion, .. } => completion,
            _ => {
                return Err(MobError::Internal(format!("unexpected submit work result")));
            }
        };
        Ok(MemberDeliveryReceipt {
            identity,
