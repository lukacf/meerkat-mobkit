...
pub trait MobProvisioner: Send + Sync {
    ...
    async fn start_flow_step(
        ...
        self.start_turn(member_ref, req).await.map(|_| ())
    }
...
impl MobProvisioner for SessionBackend {
    ...
    async fn start_turn(
        ...
        if self.runtime_adapter.is_some() {
            ...
            self.execute_runtime_input(&session_id, input, req.event_tx)
                .await?;
            return Ok(None);
        }
    ...
    async fn admit_turn(
        ...
        if self.runtime_adapter.is_some() {
            self.ops_adapter
                ...
                .await?;
            let input = Self::runtime_input_from_turn_request(&req);
            self.admit_runtime_input(&session_id, input, req.event_tx)
                .await?;
            return Ok(None);
        }
    ...
    async fn trusted_peer_spec(
        ...
        let mut trusted_peer = Self::trusted_peer_spec(fallback_name, fallback_peer_id)?;
        if let Some(comms) = self.comms_runtime(member_ref).await
            && let Some(address) = comms.advertised_address()
        {
            trusted_peer.address =
                meerkat_core::comms::PeerAddress::parse(address.as_str()).map_err(|error| {
                    MobError::WiringError(format!(
                        "invalid advertised peer address for '{fallback_name}': {error}"
                    ))
                })?;
        }
        self.ops_adapter
        ...
        match member_ref {
            MemberRef::BackendPeer {
                ...
                let response: super::bridge_protocol::BridgeDeliveryResponse = self
                    .send_bridge_command_typed(&peer, &command, Duration::from_secs(120))
                    .await?;
                let completion = response.completion.clone();
