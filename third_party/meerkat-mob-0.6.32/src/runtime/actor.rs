...
impl MobActor {
    ...
    fn peer_only_spec_from_parts_with_name(
        peer_name: &str,
        peer_id: &str,
        address: &str,
        context: &'static str,
    ...
    fn peer_only_spec_for_binding_with_name(
        binding: &crate::RuntimeBinding,
        peer_name: &str,
        context: &'static str,
    ) -> Result<TrustedPeerDescriptor, MobError> {
        match binding {
            crate::RuntimeBinding::External {
                ...
                pubkey,
                ..
            } => Self::peer_only_spec_from_parts_with_name(
                peer_name, peer_id, address, context, *pubkey,
            ),
            crate::RuntimeBinding::Session => Err(MobError::Internal(format!(
                "{context}: peer-only runtime spec requested for session binding"
            ))),
    ...
    async fn finish_submit_work_dispatch(
        ...
        match completion {
            ...
            SubmitWorkDispatchCompletion::AwaitTurnAdmission {
                ...
                member_ref,
                req,
            } => provisioner.admit_turn(&member_ref, *req).await.map(|_| ()),
            SubmitWorkDispatchCompletion::AwaitTurnCompletion {
                provisioner,
                member_ref,
                req,
            } => provisioner.start_turn(&member_ref, *req).await.map(|_| ()),
        }
    }
