...
"$CARGO_CMD" build -p meerkat-mobkit --bin mobkit_flow_editor

LOG_FILE="$(mktemp -t mobkit-flow-editor-rkat-e2e.XXXXXX.log)"
"$ROOT/target/debug/mobkit_flow_editor" --listen "$ADDR" >"$LOG_FILE" 2>&1 &
SERVER_PID=$!
...
for _ in $(seq 1 100); do
  if curl -fsS -X POST "$RPC_URL" \
    ...
    >/dev/null 2>&1; then
    MOBKIT_FLOW_EDITOR_RPC_URL="$RPC_URL" npm --prefix "$ROOT/flow-editor" run "$NPM_SCRIPT" --silent
    exit 0
  fi
