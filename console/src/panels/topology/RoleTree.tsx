  ...
  return (
    <div className="topo-roletree">
      <div className="topo-roletree__row">
        <button
          type="button"
          className={`topo-roletree__mob ${rootHot ? "is-hot" : ""}`}
      ...
      {expanded.__root && graph.roles.map((role) => {
        ...
        return (
          <div className="topo-roletree__section" key={role}>
            <button
              type="button"
              className={`topo-roletree__role ${sectionHot ? "is-hot" : ""}`}
              onClick={() => toggle(role)}
            >
              <span
                className="topo-roletree__chevron"
              ...
              <div className="topo-roletree__pod">
                {list.map((agent) => {
                  ...
                  return (
                    <div
                      key={agent.id}
                      className={`topo-roletree__agent ${isHot ? "is-hot" : ""}`}
                      data-testid={`topology-node:${agent.id}`}
                      title={`${agent.id}${agent.state ? " · " + agent.state : ""}`}
                    >
                      <span
