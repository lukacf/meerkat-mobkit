...
| `agent_memory` | Boolean or object | Enables identity-first agent memory injection for roster-backed agents. `true` uses contextual recall defaults and stores markdown under `<persistent_state>/agent-memory`; an object may set `realm`, `selection` (`"always"` or `"contextual"`), `max_entries`, and `instruction_header`. |
...
`agent_memory` is separate from `mobkit/memory/*`. It is identity-first build context: during create/resume/materialize, MobKit asks an `AgentMemoryProvider` for memories keyed by `AgentIdentity` and injects a bounded, quoted observation summary into `AgentBuildDraft.additional_instructions`. The bundled gateway provider stores inspectable markdown files by realm and identity under `<persistent_state>/agent-memory`; the path is server-owned and cannot be overridden from SDK init input.
