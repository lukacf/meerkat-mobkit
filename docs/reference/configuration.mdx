...
close the process. The current 310-second horizon covers an already-admitted
provider callback, runtime event and mob drains, the final lease-release
callback, and bounded response-delivery/reaping margin. Older and custom
gateways that do not advertise the handshake retain the EOF shutdown protocol.
