...
| `identity_bootstrap_mode` | Object | Controls roster materialization for identity-first gateways. Omitted or `{ "mode": "eager_materialize" }` preserves synchronous eager startup. `{ "mode": "lazy_materialize" }` registers metadata and materializes on use. `{ "mode": "lazy_with_background_warm", "concurrency": 2 }` returns after metadata registration and warms in a tracked background task; concurrency must be `1..=16`. Lazy modes require a roster provider. |
