...
MobKit is a single Rust crate (`meerkat-mobkit-core`) with a React console, designed as a companion to the Meerkat multi-agent runtime. It handles the operational concerns that surround agent execution: startup, module supervision, delivery, scheduling, gating, memory, sessions, and administration.
...
The runtime composes three Axum router layers:
...
| SSE | `/interactions/stream` | Agent interaction |
