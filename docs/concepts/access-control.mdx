...
<Warning>
**A control listener without grants authorizes nothing.** `start_control_listener` (and today's `--control-listen`) serve every peer that can reach the endpoint, on every verb, against every member - which is why the listener should stay on a trusted network until grants are configured. Installing a table is what turns the channel from "reachable means permitted" into "granted means permitted". An **empty** table is not the same as no table: an empty table refuses everyone.
</Warning>
...
<Note>
The gateway config key that populates this table from `runtime_options` lands with the gateway plumbing; until then, grants are configured by embedders through `start_control_listener_with_authorizer`. `ControlAuthorizer::from_toml` already accepts the `[control_grants]` section shown above and returns an open authorizer when the section is absent, so a config that never mentions grants keeps its current behaviour.
</Note>
