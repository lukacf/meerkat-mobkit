...
Modules are the building blocks of MobKit's operational layer. Each module is an MCP server or subprocess executable that provides a specific capability -- routing, delivery, gating, or memory. MobKit supervises their lifecycle and routes calls to them. Durable scheduling is owned separately by Meerkat's `ScheduleService`.
...
DiscoverySpec {
    ...
    modules: vec!["router", "delivery", "gating", "memory"],
}
```
...
MobKit ships with four core module types, each backed by an MCP server:
...
Scheduling is not a module-plane capability. Use the durable scheduling tools and `ScheduleService` described in [Scheduling](/concepts/scheduling).
