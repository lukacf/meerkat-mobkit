...
Write an identity-scoped agent memory record for later build-context injection. This is separate from `mobkit/memory/*`, which is the operational assertion ledger.
...
Read identity-scoped agent memory records from the configured agent memory provider. This exposes the same durable record set used for build-context injection.
...
| `query_terms` | `string[]` | no | Terms used by contextual providers |
