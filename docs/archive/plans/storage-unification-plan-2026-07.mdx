---
title: "Historical record - MobKit storage unification plan"
description: "Archived July 2026 implementation plan. Preserved for design history; not current operational guidance."
icon: "database"
---
...
Two assumptions recorded in the plan were retired after implementation. The
H3 `adopt_legacy_session` checkpoint-stamp mechanism, including its
lazy-at-restore path and maintenance tooling, was retired in 0.8.11. That is
separate from the supported one-time 0.8.10 envelope importer and the
first-delta conversion of blob-canonical continuity sessions. Meerkat's
`StorageMigrator` remains a read-only diagnosis seam; it has no mutation half
or extension. MobKit's mutating maintenance operations are concrete
`MobKitStorageMigrator` methods and the `mobkit_gateway storage-migrate` and
`storage-prune` commands.

For current behavior, see [storage layout and durability](/reference/configuration#storage-layout-and-durability),
[session persistence](/concepts/sessions#on-disk-layout-and-durability), and
[storage RPC methods](/api/rpc#storage-methods).
</Warning>
