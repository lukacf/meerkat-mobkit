...
gateway launch, and the bump is reversible — `storage-downgrade --apply`
re-materializes head+rows back into whole-document blobs, drops the
channel, and rewinds the ledger, so rollback keeps every post-upgrade turn
instead of restoring a pre-upgrade backup; a `ContinuityStore`
default-method rollback defect found
