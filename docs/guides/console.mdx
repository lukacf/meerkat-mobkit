...
<Steps>
  <Step title="Open SSE stream">
    Post to `/interactions/stream` with `member_id` before sending. This ensures no events are missed if the agent responds quickly.
  </Step>
  <Step title="Send message">
    Call the `mobkit/send_message` RPC method:
    ```json
    { "member_id": "domain:billing", "message": "What is the current status?" }
    ```
    Returns `{ "accepted": true, "member_id": "...", "session_id": "..." }`.
  </Step>
  <Step title="Receive response frames">
    The SSE stream delivers frames filtered by session ID:
    ```
    event: text_delta
    data: {"type":"text_delta","delta":"The current status is..."}
    ```
    Infrastructure events (`run_started`, `turn_completed`, etc.) are shown in the activity rail but filtered from the conversation transcript.
  </Step>
</Steps>
