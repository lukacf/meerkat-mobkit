...
selected agent as its background agent. Voice starts from a summary of that
agent's context; it does not change the background agent's text model.
...
Startup distinguishes **Preparing context** from **Connecting audio**. Context
preparation is a separate, tool-free summary request using the background
...
it. Preparation timeouts identify the gateway stage rather than blaming
