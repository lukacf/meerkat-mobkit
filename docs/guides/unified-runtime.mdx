...
// Interaction SSE (agent messaging)
let sse = interaction_sse_router(mob_runtime);

// Compose into a single app
let app = frontend.merge(json_api).merge(sse);
```
...
| `/interactions/stream` | POST | Send message, receive SSE stream |
