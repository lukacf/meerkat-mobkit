...
When the runtime needs to invoke a module - for example, for a delivery send
or memory conflict lookup - it routes the call through `route_module_call`:
...
Core module-backed operations (router, delivery, and memory) communicate over MCP tool calls. The MCP boundary:
...
<Steps>
  ...
  <Step title="Capabilities handshake">
    ...
    The runtime validates the contract version matches `MOBKIT_CONTRACT_VERSION` (`"0.5.0"`).
  </Step>
...
Subprocess modules receive JSON-RPC requests routed by method name. Durable
scheduling does not use this module path; it is owned by `ScheduleService`.
