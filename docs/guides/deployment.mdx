...
MobKit mints the id of every object under `mobkit/blob/*` and behind
`/blobs/{id}` as `sha256(media_type || 0x00 || raw_bytes)` over the decoded
image bytes (`meerkat-mobkit/src/blob_store.rs`, `compute_blob_id`). Meerkat
mints its content blob ids as `sha256(canonical_media_type || 0x00 ||
base64_text)` over the base64 string it stores. Both render as
`sha256:<64 hex digits>`, so the same PNG carries two different, visually
indistinguishable ids. Compare MobKit ids only with MobKit ids and meerkat ids
only with meerkat ids; a lookup across the two stores misses. The side by side
