...
payload and attests only an exact match. A durable fork of a transcript that
holds such a reference then stores the image under its meerkat content address
and points the child at it, leaving the source transcript and object in place,
and realtime history hydration accepts the reference. The side by side
