...
| `.meerkat_config(path)` | Meerkat host `config.toml` (`[self_hosted]`, `[realm]`, `[models]`) forwarded as `runtime_options.meerkat_config_path`; a missing or malformed file refuses init. See [Self-hosted models](/reference/configuration#self-hosted-models-and-the-meerkat-host-config) |
