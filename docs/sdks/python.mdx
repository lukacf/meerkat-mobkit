...
| `send_message(member_id, message)` | `None` | Fire-and-forget message delivery |
...
```python
from meerkat_mobkit.events import MobEvent, AgentEvent, EventStream
```

```python
# Send a message and observe responses via SSE bridge
await handle.send_message("agent-1", "Hello")
bridge = runtime.sse_bridge()
async for event in bridge.agent_events("agent-1"):
