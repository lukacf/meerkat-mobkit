...
from `build_agent`: the gateway applies only `additional_instructions`,
`labels`, `resume_session_id`, and the registered `tools` from what you
return, and ignores a host-set `profile_name`. Per-profile tools are therefore
