...
`mobpack_deploy(document, execute=True)` writes the archive and runs `rkat mob deploy` on the host — gated by the surface's advertised `authoring_capabilities` and, on ABAC-enforced runtimes, the caller's `mobpack.deploy` grant. See the [Flow Editor guide](/guides/flow-editor).
